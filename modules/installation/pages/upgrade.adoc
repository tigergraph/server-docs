...
It is supported only for maintenance updates (for example, 4.2.x to 4.2.1, or 4.3.1 to 4.3.3). Upgrading from 4.2.0 to 4.3.0 or another major version is not supported.
...
*Query Processing and Failure Handling During Rolling Upgrade*

Queries usually continue to work while a rolling upgrade is in progress, but can fail in these scenarios:

* If a query is sent to a replica that is about to be upgraded, it may fail because the services on that replica will be restarted. The query will timeout if the replica doesn’t respond in time.
...
* Avoid running long queries during upgrade. Break them into smaller parts or optimize them when possible.
...
If users selects `y/Y` the installer will automatically switch TigerGraph to the latest maintenance version.
