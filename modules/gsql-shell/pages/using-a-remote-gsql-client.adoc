...
Since TigerGraph Cloud uses a different xref:master@cloud:security:manage-db-users.adoc[authentication scheme] from TigerGraph Server, the steps for using the GSQL client for a cloud solution are slightly different.
