...
+
If you already have a running external Zookeeper instance or external Kafka cluster for CDC, this step can be skipped.
+
...
+
**Once the TigerGraph CDC service is running, test it by making updates to an existing graph with xref:gsql-ref:querying:data-modification-statements.adoc[].**
+
Statements like:
+
