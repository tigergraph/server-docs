...
You can append a request header `gsql-atomic-level` to the request and set its value to `atomic` to make the request an atomic transaction.
...
curl -X POST -H 'gsql-atomic-level:atomic' --data-binary @vertices.json http://localhost:9000/graph/social?automic_post=true
----
