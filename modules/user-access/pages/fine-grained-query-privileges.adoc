...
[NOTE]
====
`OWNERSHIP` is a special privilege introduced since 4.1.0. It should be granted to the user created this query automatically. 
...
At the same time, this entity can grant the query privileges to other entities, including the `OWNERSHIP` privilege. Once the `OWNERSHIP` privilege is granted to another entity, the previous owner should lose `OWNERSHIP` privilege on this query.

Users with the `superuser` role should hold the `OWNERSHIP` privilege on all queries in all graphs implicitly.

Users with the `admin` role in one graph should hold the `OWNERSHIP` privilege on all queries in this graph implicitly.
====
...
** `OWNERSHIP` privilege can only be granted at individual query level and only be granted to one entity, e.g.: a user or a user-defined role.
...
[NOTE]
====
After this step, `u1` becomes the owner of `q3`, and owns all privileges on `q3`. `u1` can read, update, drop, install and execute query `q3`. At the same time, `u1` can grant the `OWNERSHIP` privilege to another entity as well.
====
