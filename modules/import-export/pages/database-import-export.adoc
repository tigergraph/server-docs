...
Rather than creating a single export zip file, the `EXPORT QUERY` command will create a file for each machine. Before exporting, specific folders must be created on each server using the following commands:
