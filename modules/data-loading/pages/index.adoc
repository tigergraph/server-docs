...
(xref:load-from-warehouse.adoc#_bigquery[BigQuery], xref:load-from-warehouse.adoc#_snowflake[Snowflake], and xref:data-loading:load-from-warehouse.adoc#_postgresql[PostgreSql]).

== xref:load-from-kafka.adoc[Load Data from an External Kafka Cluster]
Instructions for loading records from Kafka topics including CSV, JSON and Avro formats.
With additional instructions on xref:avro-validation-with-kafka.adoc[Avro Data Validation through KafkaConnect] and how to xref:data-loading:kafka-ssl-security-guide.adoc[Set up SSL on Kafka] or xref:data-loading:externalizing-kafka-configs.adoc[Externalize Kafka Configs].

[NOTE]
====
This is an improved connector for Kafka sources introduced in 4.1.3+.
The xref:load-from-kafka-mm.adoc[previous Kafka source connector, using MirrorMaker], is still supported but deprecated.
====
