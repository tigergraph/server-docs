...
¦The `.flip()` function behaves differently in Single/Dist mode compared to UDF mode. In UDF mode, values are updated, but in Single/Dist mode, they remain unchanged. This is caused by the block mutator function being called inside the PRINT statement. To resolve this, use `.flip()` inside the POST-ACCUM block instead.
¦4.1.3
a¦TBD
¦TBD
