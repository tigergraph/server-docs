...
|Kafka.RetentionSizeGB |The minimum size (in GB) of Kafka log files for *infrastructure services* before they are eligible for deletion. This setting does not control 
retention for GPE or GSE data. +
...
Before v4.1.2, this must be in PKCS#8 format (start with ``BEGIN PRIVATE KEY'').
