...
|Enables IAM role-based authentication for AWS S3 backup and restore. When enabled, TigerGraph uses the EC2 instance profile or EKS IAM Roles for Service Accounts (IRSA) instead of access keys.
|`false`

|System.Backup.S3.Region
|AWS region used when signing requests for the S3 bucket.
|`+nan+`
...
|System.Backup.CompressProcessNumber | Number of concurrent processes for compression during backup (default `10`).

 `0` means the number of processes used to compress is equal to the number of CPU cores on each node.
| `10`
...
----
+
.For access key authentication, configure the access key ID
[console]
----
...
----
+
.Alternatively, to use IAM role-based authentication, configure the AWS Role ARN and enable IAM authentication.
+
...
[console]
----
gadmin config set "System.Backup.S3.IAMAuth" "true"
----
+
.If your S3 bucket is located in a specific AWS region, configure the region used to sign S3 requests.
+
.Set the AWS S3 region
[console]
----
gadmin config set "System.Backup.S3.Region" "<aws-region>"
----
+
