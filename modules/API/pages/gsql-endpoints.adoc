...
=== Get loading job status (multiple jobs)

`GET /gsql/v1/loading-jobs/status`

Get the status of one or more loading jobs, equivalent to GSQL
...
=== Get loading job status (one job)

`GET /gsql/v1/loading-jobs/status/{jobId}`

Get the status of one loading job.
