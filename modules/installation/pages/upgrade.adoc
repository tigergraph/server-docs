...
. Follow the appropriate upgrade procedure for your environment, such as xref:#_upgrading_a_crr_system[Upgrading a CRR system] or the version-specific upgrade paths below.
...
. First perform a backup xref:backup-and-restore:index.adoc[backup] on your existing installation.
. *Restore* your installation with `gadmin backup restore` to rebuild it.
...
. xref:backup-and-restore:backup-and-restore.adoc[Back up] your TigerGraph instance using `gadmin backup`.
