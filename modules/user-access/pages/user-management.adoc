...
IMPORTANT: All usernames are treated as a `STRING` in `GSQL`
regardless of the syntax structure, format, or characters used.
Including email syntax (Ex: a@b.com)
...
NOTE: Backquotes ( ``` ) may be used as a delimiter to enclose the username which does not follow
the pattern `[a-zA-Z_][a-zA-Z0-9]*`. The delimiters are not stored internally.

== Username Rules for v3.9.2 and below

NOTE: For versions 3.9.2 and below please follow these guidelines when creating a username.
...
. It also cannot start with a dot ( `.` ) or have multiple dots ( `...` or `a.b..c...` ) in a sequence.

NOTE: You may use non-ascii characters, such as Chinese and Kanji characters.
