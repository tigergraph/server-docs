...
* *Local files*: `[{"name":"load_job","sys.data_root":"/tmp","dataSources":[{"filename":"f","path":"./data","name":"file:m1"}]}]`
...
For local file loading in a cluster, the `name` field in `dataSources` can be used to specify where the file is located:

* `file` or `file:any` loads from any node (default behavior).
* `file:all` requires the file to be available on all nodes.
* `file:<machine>` (for example, `file:m1`) loads from a specific node.
* `file:<m1>\|<m2>\|<m3>` allows loading from any of the listed nodes.

The optional `hostname` field is also supported for backward compatibility. If both `name` and `hostname` specify a node, the value in `name` takes precedence. If neither is specified, the loader treats the host as `any`.
...
`[{"name":"load_job","sys.data_root":"/tmp","verbose":true,"dryrun":true,"interval":1,"maxNumError":1,"maxPercentError":1,"dataSources":[{"filename":"f","path":"./data","name":"file:m1"}],"configs":{"CONCURRENCY":2,"eof":"false","BATCH_SIZE":3}}]`
--
Sample Response::
+
--
