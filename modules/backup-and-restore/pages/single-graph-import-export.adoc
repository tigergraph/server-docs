...
To see how to Export/Import all graphs in a database at once refer to xref:database-import-export.adoc[].

[IMPORTANT]
====
Import/export is a complement to xref:backup-and-restore:index.adoc[], but not a substitute.
====
...
* Additionally, by default, it will *NOT* export, unless specified option `--USERS`:
** Users
** Roles
** Secrets
** And tokens
...
(-S | --SCHEMA | -T | --TEMPLATE | -D | --DATA | -U | --USERS | -P | --PASSWORD pwd)
    ...
    -S, --SCHEMA        Only schema and attribute indices will be exported

    -T, --TEMPLATE      Schema, Queries, Loading Jobs, UDFs, and Data Sources
                        will be exported (no data or users)

    -D, --DATA          Include vertex and edge data export (must be used with -S or -T)

    -U, --USERS         Include Users, Permissions, Secrets, and Tokens
                        (must be used with -S or -T)
...
[NOTE]
====
* `-S` and `-T` cannot be used together.

* `-D` and `-U` must be used with either `-S` or `-T`. Using them alone will result in an error.

* If none of `-S`, `-T`, `-D`, or `-U` are specified, all content is exported, including schema, data sources, loading jobs, queries, UDFs, data, and users.
====
