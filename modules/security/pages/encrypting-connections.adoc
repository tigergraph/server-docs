...
TigerGraph's Nginx supports both TLS 1.2 and TLS 1.3, with TLS 1.3 used by default for both client-facing and internal cluster communication. TLS 1.3 is the latest version of the TLS protocol, offering stronger security and faster connection establishment.
...
Changes to `Nginx.ConfigTemplate` do not persist across upgrades. The template is backed up and reset during the upgrade process to incorporate any version-specific changes. Users must manually reapply their modifications to the new template version, resolving any conflicts as needed.
