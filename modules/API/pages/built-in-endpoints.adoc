...
[NOTE]
====
This RESTPP endpoint is the recommended endpoint for running installed queries because it avoids the additional processing performed by the GSQL endpoint.

Use the xref:api:gsql-endpoints.adoc#_run_query[GSQL Run query endpoint] only if you specifically require its additional functionality.
====
