...
:description: An overview of HA concepts in TigerGraph and how to configure an HA cluster.nn
:stem: latexmath
:partition: partition
:bucket: bucket
...
[TIP]
====
HA clusters are designed to handle node-level failures.
To tolerate region-level failures, please refer to xref:region-aware.adoc[Region Aware Cluster Configuration].
====
...
* Configure HA settings xref:get-started:install/bare-metal-install.adoc#_edit_configuration_file[during installation].
...
Configuring a HA cluster is part of platform xref:get-started:install/bare-metal-install.adoc[installation].
