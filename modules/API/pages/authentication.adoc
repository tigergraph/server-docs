...
. Each user link:../../admin/admin-guide/user-access/managing-credentials.md#create-a-secret[can create one or more *secrets* (unique pseudorandom strings)]. Each secret is associated with a particular user and the user's privileges for a particular graph.
. Anyone who has this secret can invoke a special REST endpoint to link:built-in-endpoints.md#request-a-token-get[generate authorization *tokens*] _**_(other pseudorandom strings).
