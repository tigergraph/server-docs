...
and Redshift
* Open table format:
xref:load-from-spark-dataframe.adoc#_load_data_from_delta_lake[Delta Lake],
xref:load-from-spark-dataframe.adoc#_load_data_from_iceberg[Iceberg]
and xref:load-from-spark-dataframe.adoc#_load_data_from_hudi[Hudi]

[IMPORTANT]
====
The legacy xref:data-loading:spark-connection-via-jdbc-driver.adoc[Spark Connection Via JDBC Driver] is deprecated. Please migrate to this new connector.
====
...
=== Download the JARs
This connector can be downloaded from the Maven central repository for
https://central.sonatype.com/artifact/com.tigergraph/tigergraph-spark-connector_2.12/overview[Scala 2.12] and https://central.sonatype.com/artifact/com.tigergraph/tigergraph-spark-connector_2.13/overview[Scala 2.13]
...
[NOTE]
====
`OAuth2` authentication for Spark was originally added in *4.1.3* and backported to *3.10.4*.
So, in version *3.10.4*, TigerGraph's Spark Connector supports `OAuth2` authentication, enabling it to request JWT tokens from third-party Identity Providers (IdPs) like `Azure AD` and `Auth0`, enhancing security and token management for Spark jobs.
====
