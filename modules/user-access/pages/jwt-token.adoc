...
Starting from 4.1.0, the token generated by GSQL API is JWT Token and can be used on GSQL and RESTPP server for authentication.
 ...
 * `secret`: the `secret` represents the user who owns so there is no need user login if secret set in the payload
    if there is no secret specified in the payload a secret will be generated automatically
                  
 * `graph`: the `graph` means the scope the token can be used
    if there is no graph specified the token is global token which is applied to all endpoints
    
 * `lifetime`: the `lifetime` means the duration of token can be used and  the time unit is in ms 
    if there is no lifetime specified then the default time is 7 days

==== Required Privilege:
If `secret` is specified in the payload, there is no need for user login and if `graph` is specified in the payload, the user must have access to the data of the graph.

=== drop JWT Token
Since the JWT token cannot be deleted as it has been created, there is a block list to include JWT tokens that are restricted to access on RESTPP and GSQL server.
...
==== Required Privilege:
There is no privilege needed since drop JWT token operation is for current user who logins.
