...
To see how to Export/Import selected individual graphs in a database refer to xref:backup/single-graph-import-export.adoc[].
...
[IMPORTANT]
====
Import/export is a complement to xref:backup/index.adoc[], but not a substitute. See the xref:#_known_issues[] for additional considerations.
====
...
[NOTE]
====
xref:access/ldap.adoc#_proxy_users[Proxy users] cannot be exported.
====
...
Therefore, create a complete backup beforehand in case you need to restore the database: xref:backup/backup-cluster.adoc[]
