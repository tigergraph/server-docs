...
TigerGraph requires two external ports to be exposed: 22 and 14240. 
All other ports, including port 9000 (internal since version 4.1), are used for internal communication within a cluster and must be open within the cluster.
...
|Internal use only since version 4.1. Do not use this port for external or custom applications to avoid conflicts.
|9166
