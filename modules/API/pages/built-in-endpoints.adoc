...
In the sample request below, three filters are used: `from`, `to` and `what`.

The timeframe specified by `from` and `to` indicate start and end times of `1618957536` to `1619023346`.
...
In the below example, `what` and `latest` are used. The request asks for the `10` latest data points regarding ``mem``ory usage:
...
Users can use either `secret` or their username and password to generate a token. If no secret is supplied and the user chooses to use their username and password instead, then the parameter `graph` becomes required.
...
| `graph`
| Yes
| The name of the graph from which the schema will be retrieved.
...
By default, the `+POST /graph/{graph_name}+` endpoint is not atomic, meaning it is not all-or-nothing. If something goes wrong while the request is being made,  the request data can be partially received by the database.
...
If you have a TigerGraph HA cluster, you can specify a query to run on a particular replica with the HTTP header `GSQL-REPLICA`. The value of the header needs to be an integer within the range `[1 - (cluster replication factor)]`. If you supply a invalid value for the header, the request will return an error.
...
| `url`
| No
| The endpoint on which to abort running queries. You must specify the base of the endpoint's URL, but then use a wildcard to allow for different parameters. For example, to abort all running queries for the endpoint `/sleepgpe`, use `url=/sleepgpe.*`
|===
