...
Point-in-Time Restore, introduced in version 3.11, enables users to restore the database to a previous time point using previously made backups, even though no backup was conducted at exactly that time point.
This feature relies on the user having made one or more xref:differential-backups.adoc[differential backups].
