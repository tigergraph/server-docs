...
When using a `POST` request to run an installed query, the query parameters are passed in through the request body and link:intro.md#formatting-data-in-json[encoded in JSON format]. The formatting rules for the JSON payload is the same as link:../gsql-ref/querying/query-operations.md#parameter-json-object[using JSON to pass in parameters in the `RUN QUERY` command].
...
Other versions of pathfinding algorithms are available in the link:../../graph-algorithm-library/#path-algorithms[GSQL Graph Algorithm Library].
