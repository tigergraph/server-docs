...
If you have the Organization Admin role for your TigerGraph Cloud organization, xref:cloud:security:manage-database-users.adoc[generate a database username-password pair] to use as credentials for your connection.

If you do not have the Organization Admin role, ask for your organization admin to create a pair for you.

If your TigerGraph Cloud solution is created before August 18th, 2022, you won't be able to create username/password pairs.
Instead you should Use Admin Portal to xref:gui:admin-portal:management/user-management.adoc[generate a secret] for your user account.
...
<4> Replace `<username>` and `<password>` with the username and password of the TigerGraph user you want to be logged in as.
If your solution was created before August 18th, 2022, replace `<username>` with `\__GSQL__secret`, and replace `<password>` with your secret. 
