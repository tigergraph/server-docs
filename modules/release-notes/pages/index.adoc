...
¦ A GPE crash occurs when a `DELETE` operation is performed on multi-edge types without specifying a discriminator.
¦ 3.10.0
¦ Use the `delete-all` operator introduced in 4.1.1 to delete multi-edges without causing a GPE crash. This operator ensures that a GPE crash does not occur when no discriminator is specified. For more details, refer to the xref:@tigergraph-server:system-management:change-data-capture/cdc-message-example.adoc#_possible_operators_for_cdc_message_with_type_edge[CDC documentation] for the `delete-all` operator.
¦ 4.1.1
