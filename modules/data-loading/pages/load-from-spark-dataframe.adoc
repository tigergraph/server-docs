...
Alternatively, The TigerGraph Spark Connector provides a xref:tigergraph-server:data-loading:load-from-spark-dataframe.adoc#_direct_upsert_from_spark_dataframe[Direct Upsert] feature that enables direct upsert of vertices and edges from a Spark DataFrame to a TigerGraph database without the need to create a GSQL loading job beforehand.
This is particularly useful for ad-hoc data loading, small datasets, or when the data schema is dynamic.
