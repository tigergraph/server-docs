...
. Apply the configuration and restart the services that depend on it:
+
.Enabling REST{pp} Authentication
+
[source,bash]
----
$ gadmin config apply --restart-deps -y
----
+
This command restarts `restpp`, `nginx`, `gui`, and `gsql`, which depend on the `RESTPP.Factory.EnableAuth` configuration parameter. The services are restarted automatically in the correct dependency order.
