...
This feature maintains the same xref:backup/backup-configuration.adoc[] as before.
...
With online backup in place, users can seamlessly execute post requests via the xref:reference:REST/index.adoc[REST API] during the backup process.
This means that even as critical operations continue, the database is concurrently backed up, ensuring continuous data availability and preventing the inability to xref:reference:REST/upsert-rest.adoc[upsert data to the database].
====
