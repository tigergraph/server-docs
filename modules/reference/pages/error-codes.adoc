...
If the system was handling a user request, the status code and message will be in the JSON response (see link:../dev/gsql-ref/querying/query-operations.md#gsql-query-output-format[GSQL Query Output Format] ). For internal errors, the error information may be in a log file.
