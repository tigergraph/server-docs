...
* Fixed issue where `/query_status` and `/query_result` endpoints require requests to specific cluster nodes, now making them node-agnostic for consistent access (CORE-5202).
* Fixed issue where the `pre_upgrade_check_gsql.sh` script failed silently after running `decrypt_acl_catalog`, preventing important error messages from being displayed during upgrade checks (GLE-11481).

==== Improvements

* Added the ability to configure file permissions for files generated using the `PRINT TO_CSV statement` in GSQL (GLE-11321).
