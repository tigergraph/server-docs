...
* One or more machines that meet the minimum xref:hw-and-sw-requirements.adoc[_Hardware and Software Requirements_]. If your machine doesn't have the software packages necessary, the downloadable TigerGraph package has a setup script.
...
Navigate to the `tigergraph-<version>-offline` folder and run `install.sh`  with the following commands:
...
The `tigergraph-<version>-offline` folder contains a file named `install_conf.json`. For non-interactive mode installation, the user must review and modify all the settings in the file `install_conf.json` before running the installer.
