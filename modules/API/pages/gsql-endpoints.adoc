...
* `file:any` (or simply `file`) loads the file from any node where it is available.
* `file:all` requires the file to be present on every node.
* `file:<machine>` loads the file from the specified node (for example, `file:m1`).
* `file:<m1>\|<m2>\|<m3>` allows loading from any of the listed nodes.

Choosing the appropriate node specification can help distribute loading across cluster nodes for parallel loading.
====
