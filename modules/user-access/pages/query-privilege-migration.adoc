...
TigerGraph blocks ACL privilege migrations in  4.2.1, if the ACL catalog contains query contents. Before upgrading from version 3.x, you must revoke all ACL READ privileges on queries. 
...
[NOTE]
====
If ACL read privileges are still present during upgrade, the system displays this message:
----
