...
* Fixed an issue that could crash Graph Processing Engine (GPE) if `v.getAttr()` was provided the wrong type.
* Fixed an issue that could cause crashes where multiple vertex types had the same attribute names and different positions in rare cases.
