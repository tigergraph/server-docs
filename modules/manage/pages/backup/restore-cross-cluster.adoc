...
== Prerequisites
* Your new cluster has the same xref:cluster:ha-cluster.adoc[partitioning factor] as the cluster from which the backup file was created.
...
* You have xref:backup/backup-configuration.adoc[configured backup and restore] on the new cluster.
