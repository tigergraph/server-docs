...
The web version of GSQL provides many user-friendly features such as command history, GSQL script uploading and downloading, keyboard shortcuts, GSQL command auto completes, syntax highlighting, and many more that make the GSQL shell easy to use for GSQL developers.
...
For example, if your instance is on `localhost`, visit \http://localhost:14240, log in if xref:user-access:enabling-user-authentication.adoc[authentication is enabled], and select GSQL Shell.
