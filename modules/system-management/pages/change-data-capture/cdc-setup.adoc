...
+
Now, to start the CDC service in TigerGraph use the setup configuration commands.
System.CDC.ProducerConfig
System.CDC.TopicConfig
System.CDC.Enable
MYIP=127.0.0.1
...
3.1 Make an update to data
Now, once the service is up and running, test it with data modifications to an existing graph through statements like update, running custom or built-in queries, or running a loading job.
