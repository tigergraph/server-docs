...
Please note that occasional drops in Queries Per Second (QPS) may occur during the upgrade. Additionally, services like Zookeeper (ZK), Kafka, and ETCD will not be upgraded during this process.
...
Before starting the upgrade process, ensure that long-running operations are allowed by running the following command:
...
If the upgrade fails for any reason, the error prompt will guide you on whether to continue manually or revert to the previous version. 
Use the following command if you need to manually finish the upgrade or revert:

*Manual Upgrade*: Execute the following command to restart services in sequence:
