...
Use command `gadmin metric -t cpu` to collect CPU usage by different TigerGraph services on a cluster or a node in the cluster.
...
The following command reports CPU usage by all services by m1 and m2:
...
Use command `gadmin metric -t dsik` to report diskspace usage for a cluster or select nodes in a cluster.
