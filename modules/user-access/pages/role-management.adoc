...
This page explains the procedures for various role management tasks under TigerGraph's classic role-based access control model.

[NOTE]
====
*Known Issue*: usernames beginning with a digit or containing non-alphanumeric characters cannot grant or revoke privileges.
====

For Row Policy and Object-Based privilege management, see
xref:user-access:rbac-row-policy/row-policy-overview.adoc[].
...
This will allow users with the roles `role1` and `role2` to create, read, update, drop, install and execute existing queries, as well as modify roles on the graph `example_graph`. For more information on query privileges, see xref:user-access:fine-grained-query-privileges.adoc[].
...
Use `GRANT DEFAULT` to add one or more user-defined roles to the default list for a specific query-level privilege.

=== Syntax
[source.wrap,gsql]
GRANT DEFAULT <privilege_name1> (, <privilege_name2>)*
ON ALL QUERIES
IN GRAPH <graph_name> TO <role_or_user_name1> (, <role_or_user_name2>)*
...
. The following command grants default `READ` privileges on all newly created queries in a graph to a specified role:
+
[source,gsql]
----
GSQL > GRANT DEFAULT READ ON ALL QUERIES IN GRAPH MyGraph TO ROLE analytics_reader
----
This allows `analytics_reader` to automatically receive query-level `READ` access on any new query created in `MyGraph`, without requiring additional `GRANT` commands.

. The following command grants default `EXECUTE` privileges on all newly created queries in a graph to multiple roles:
+
[source,gsql]
----
GSQL > GRANT DEFAULT EXECUTE ON ALL QUERIES IN GRAPH MyGraph TO ROLE data_scientist, app_service
----
This allows `data_scientist` and `app_service` to automatically receive query-level `EXECUTE` access on any new query created in `MyGraph`, without requiring additional `GRANT` commands.




== Revoke default roles

Use `REVOKE DEFAULT` to remove roles from the default list for a specific query-level privilege.

=== Syntax
[source.wrap,gsql]
REVOKE DEFAULT <privilege_name1> (, <privilege_name2>)*
ON ALL QUERIES
IN GRAPH <graph_name> FROM <role_or_user_name1> (, <role_or_user_name2>)*
...
. The following command revokes default `READ` privileges on all newly created queries in a graph from a specified role:
+
[source,gsql]
----
GSQL > REVOKE DEFAULT READ ON ALL QUERIES IN GRAPH MyGraph FROM ROLE analytics_reader
----
This prevents `analytics_reader` from automatically receiving query-level `READ` access on any new query created in `MyGraph`. Existing queries where `analytics_reader` already has `READ` remain unchanged.

== Show default roles

Use `SHOW DEFAULT ROLES` to inspect which roles are currently configured as defaults for each supported query-level privilege.

=== Syntax
[source.wrap,gsql]
SHOW DEFAULT ROLES (IN GRAPH <graph_name>)?
...
. The following command shows which roles currently receive default query-level privileges in `MyGraph`:
+
[source,gsql]
----
GSQL > SHOW DEFAULT ROLES IN GRAPH MyGraph
----
This displays all roles and users configured to automatically receive `READ` or `EXECUTE` privileges on newly created queries in `MyGraph`.
