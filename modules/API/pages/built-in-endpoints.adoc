...
== Composite-key IDs in RESTPP endpoints

When a vertex type has a composite key, the following RESTPP endpoints support object, array, and comma-separated string formats for vertex IDs:

* `POST /restpp/graph/{graph_name}/data_query`
* `POST /restpp/shortestpath/{graph_name}`
* `POST /restpp/allpaths/{graph_name}`

=== Supported ID formats

A composite key can be specified in any of the following formats:

* *Object form:* A JSON object whose keys are the composite-key attribute names. All composite-key attributes must be present, and field order does not matter.
+
[source,json]
----
{ "type": "Transaction", "id": { "acct_id": "A123", "seq_no": 42 } }
----

* *Array form:* A JSON array containing the key values in the order in which the composite key is defined in the schema. The number of values must match the number of composite-key attributes.
+
[source,json]
----
{ "type": "Transaction", "id": [ "A123", 42 ] }
----

* *Comma-separated string form:* A comma-separated string containing the key values in the order in which the composite key is defined in the schema.
+
[source,json]
----
{ "type": "Transaction", "id": "A123,42" }
...
Values in all three forms are converted to text for the key. If a key value contains a comma, enclose that value in double quotes. Use the object or array form when a key value may contain a comma to avoid ambiguity.

== RESTPP Reload Behavior

RESTPP reload refreshes RESTPP configurations and endpoint metadata when catalog changes affect RESTPP functionality.

RESTPP reload may be triggered by operations such as:

* User authentication or privilege changes
* Endpoint changes
* Schema changes
* Workload queue configuration changes
* Datasource removals

For example, installing a query triggers a RESTPP reload because a new REST endpoint is added for the query.
However, creating a query without installing it does not trigger a RESTPP reload because no REST endpoint is added.

During RESTPP reload, RESTPP dynamically refreshes configurations and endpoints without requiring a manual service restart.

Requests that have already been accepted are not affected during the reload process. However, new requests may not be accepted temporarily until the RESTPP reload process finishes.
...
=== License expiration metric
`tigergraph_license_days_left`

Indicates the number of days remaining until the TigerGraph license expires. Any fraction of a day will be omitted. A value of -1 indicates that the license has either expired or is invalid.
...
[.wrap.console]
----
curl -s http://localhost:14240/informant/metrics | grep license
----
--
Response::
+
...
[,text]
----
# HELP tigergraph_license_days_left Days until license expiration. Any fraction of a day will be omitted. -1 means the license has expired or is invalid.
# TYPE tigergraph_license_days_left gauge
tigergraph_license_days_left 45
----
--
====
...
=== Get license info and usage

`GET /informant/licenseinfo`

*Server*: Informant

This endpoint returns the license information and usage. By default, the response is in JSON format. To receive the response in OpenMetrics format, use the header `Accept: application/openmetrics-text`.
...
[.wrap.console]
----
curl -X GET "http://localhost:14240/informant/licenseinfo"
----
--
Response::
+
...
----
{
  "License": {
    ...
    "StartTime": 0,
    "EndTime": 1771524779,
    "IssueTime": 1770315179,
    "Edition": "Enterprise",
    "Version": "",
    "Host": {
      "MaxCPUCore": 16,
      "MaxPhysicalMemoryBytes": 9007199254740991,
      "AllowedHardwareSignatures": [],
      "AllowedOSSignatures": [],
      "MaxClusterNodeNumber": 1024,
      "AllowedAzureInstanceID": [],
      "AllowedGCPInstanceID": [],
      "AllowedAWSInstanceID": [],
      "AllowedK8SInstanceID": [],
      "MaxTotalCPUs": 32
    },
    "Topology": {
      "MaxVertexNumber": 9007199254740991,
      "MaxEdgeNumber": 9007199254740991,
      "MaxGraphNumber": 9007199254740991,
      "MaxTopologyBytes": 9007199254740991
    },
    "GPE": null,
    "GSE": null,
    "GST": null,
    ...
    "GSQLShell": { "Enable": true },
    "GraphQL": { "Enable": true },
    "RuntimeMemory": {
      "ProcessRSS": null,
      "MaxUserResidentSetBytes": 549755813888
    },
    "Vector": {
      "MaxVectorBytes": 9007199254740991
    },
    "CRR": { "Enable": false },
    "DataHA": { "Enable": true },
    "Penalty": null
  ...
  "Usage": {
    ...
    "RuntimeMemory": {
      "UserResidentSetBytes": {
        "m1": 3457558138,
        "m2": 4811103300
      },
      "TotalUserRSS": 8268661438
    },
    "CRR": { "Enabled": false },
    "DataHA": { "Enabled": false }
  }
}
----
--
...
==== Sample request (OpenMetrics format):
...
[source,text]
----
# HELP tigergraph_license_limit_topology_bytes License limit for topology storage in bytes.
# TYPE tigergraph_license_limit_topology_bytes gauge
tigergraph_license_limit_topology_bytes 1e+15
# HELP tigergraph_license_limit_total_cpu License limit for total CPU.
# TYPE tigergraph_license_limit_total_cpu gauge
tigergraph_license_limit_total_cpu 100
# HELP tigergraph_license_limit_total_runtime_memory_bytes License limit for total runtime memory in bytes.
# TYPE tigergraph_license_limit_total_runtime_memory_bytes gauge
tigergraph_license_limit_total_runtime_memory_bytes 1e+15
# HELP tigergraph_license_limit_vector_bytes License limit for vector storage in bytes.
# TYPE tigergraph_license_limit_vector_bytes gauge
tigergraph_license_limit_vector_bytes 1e+15
# HELP tigergraph_license_usage_cpu_cores License usage for CPU cores per host.
# TYPE tigergraph_license_usage_cpu_cores gauge
tigergraph_license_usage_cpu_cores{host="m1"} 16
tigergraph_license_usage_cpu_cores{host="m2"} 16
# HELP tigergraph_license_usage_ratio_topology_and_vector_bytes Ratio of (topology + vector) usage to (topology + vector) limit.
# TYPE tigergraph_license_usage_ratio_topology_and_vector_bytes gauge
tigergraph_license_usage_ratio_topology_and_vector_bytes 3.93484e-09
# HELP tigergraph_license_usage_ratio_total_cpu Ratio of total CPU usage to limit.
# TYPE tigergraph_license_usage_ratio_total_cpu gauge
tigergraph_license_usage_ratio_total_cpu 0.32
# HELP tigergraph_license_usage_ratio_total_runtime_memory_bytes Ratio of total runtime memory usage to limit.
# TYPE tigergraph_license_usage_ratio_total_runtime_memory_bytes gauge
tigergraph_license_usage_ratio_total_runtime_memory_bytes 8.26866e-06
# HELP tigergraph_license_usage_runtime_memory_bytes License usage for runtime memory in bytes per host.
# TYPE tigergraph_license_usage_runtime_memory_bytes gauge
tigergraph_license_usage_runtime_memory_bytes{host="m1"} 3.45756e+09
tigergraph_license_usage_runtime_memory_bytes{host="m2"} 4.8111e+09
# HELP tigergraph_license_usage_topology_bytes License usage for topology storage in bytes.
# TYPE tigergraph_license_usage_topology_bytes gauge
tigergraph_license_usage_topology_bytes 2457
# HELP tigergraph_license_usage_topology_edge_number License usage for topology edge count.
# TYPE tigergraph_license_usage_topology_edge_number gauge
tigergraph_license_usage_topology_edge_number 82
# HELP tigergraph_license_usage_topology_vertex_number License usage for topology vertex count.
# TYPE tigergraph_license_usage_topology_vertex_number gauge
tigergraph_license_usage_topology_vertex_number 59
# HELP tigergraph_license_usage_total_cpu License usage for total CPU.
# TYPE tigergraph_license_usage_total_cpu gauge
tigergraph_license_usage_total_cpu 32
# HELP tigergraph_license_usage_total_runtime_memory_bytes License usage for total runtime memory in bytes.
# TYPE tigergraph_license_usage_total_runtime_memory_bytes gauge
tigergraph_license_usage_total_runtime_memory_bytes 8.26866e+09
# HELP tigergraph_license_usage_vector_bytes License usage for vector storage in bytes.
# TYPE tigergraph_license_usage_vector_bytes gauge
tigergraph_license_usage_vector_bytes 7.86722e+06
----
--
====
...
*Required privilege*: xref:user-access:rbac-row-policy/row-policy-privileges-table.adoc[Graph-level READ DATA]
...
Use `/gsql/v1/tokens` trough 14240 port instead, refer to xref:API:gsql-endpoints.adoc[] instead.
====
...
*Required privilege*: xref:user-access:rbac-row-policy/row-policy-privileges-table.adoc[Graph-level EXECUTE LOADINGJOB]
...
*Required privilege*: xref:user-access:rbac-row-policy/row-policy-privileges-table.adoc[Global-level CLEAR_GRAPHSTORE]
...
=== Retrieve multiple vertices and edges

`POST /restpp/graph/{graph_name}/data_query`

*Required privilege*: xref:user-access:rbac-row-policy/row-policy-privileges-table.adoc[Graph-level READ DATA]

For supported composite-key ID formats, see xref:built-in-endpoints.adoc#_composite_key_ids_in_restpp_endpoints[Composite-key IDs in RESTPP endpoints].

This endpoint retrieves multiple vertices and edges in a single request.

Below is a table of the fields in the request body.
...
| vertices
| vertex array
| List of vertices to retrieve. Each vertex is specified by its vertex `type` and `id`.

| edges
| edge array
| List of edges to retrieve. Each edge is specified by its edge `type`, `source`, and `target`. If the edge type has a discriminator, `discriminator` must also be provided.
|===

If a specified vertex or edge does not exist, or is blocked by a row policy, it is omitted from the `collected` results and listed under `skipped` instead.
...
[source.wrap,bash]
----
curl -X POST "http://localhost:14240/restpp/graph/socialNet/data_query" -d '
{
  "vertices": [
    { "type": "VidUser", "id": "0" },
    { "type": "VidUser", "id": "3" },
  ...
  "edges": [
    {
      "type": "User_Video",
      "source": { "type": "Video", "id": "0" },
      "target": { "type": "VidUser", "id": "0" },
      "discriminator": { "rating": 6, "date_time": 15000 }
    },
    {
      "type": "User_Video",
      "source": { "type": "Video", "id": "0" },
      "target": { "type": "VidUser", "id": "3" },
      "discriminator": { "rating": 8, "date_time": 16000 }
    },
    {
      "type": "User_Video",
      "source": { "type": "Video", "id": "0" },
      "target": { "type": "VidUser", "id": "898" },
      "discriminator": { "rating": 6, "date_time": 33000 }
    }
  ]
}'
----
      ...
      "edges": [
        {
          ...
          "attributes": { "rating": 6, "date_time": 15000 }
        },
        {
          "e_type": "User_Video", "from_id": "0", "from_type": "Video",
          "to_id": "3", "to_type": "VidUser", "directed": false,
          "attributes": { "rating": 8, "date_time": 16000 }
        }
      ]
    },
    "skipped": {
      "vertices": [
        { "v_id": "898", "v_type": "VidUser" },
        { "v_id": "987", "v_type": "VidUser" }
      ],
      "edges": [
        {
          "e_type": "User_Video", "from_id": "0", "from_type": "Video",
          "to_id": "898", "to_type": "VidUser",
          "discriminator": { "rating": 6, "date_time": 33000 }
        }
      ]
    }
...
| `include_vector`
| No
| Takes a boolean value. If the value is true, vector embedding attribute values are included in the output. Default is `false`.

| `include_endpoints`
| No
| Takes a boolean value. If the value is true, the source and target vertices of each retrieved edge are included in the output. Default is `false`.
|===
...
If the edge type isn't defined with a xref:{page-component-version}@gsql-ref:ddl-and-loading:defining-a-graph-schema.adoc#_discriminator[discriminator], the source, target and edge type uniquely identify an edge.
...
This RESTPP endpoint executes installed queries directly. The GSQL xref:gsql-endpoints.adoc#_run_query[Run query endpoint] internally forwards requests to this endpoint as part of the standardized GSQL API introduced in TigerGraph 4.1.
...
For a comparison of the RESTPP and GSQL endpoints, including differences in parameter formats and request processing, see xref:gsql-endpoints.adoc#_comparing_the_restpp_and_gsql_query_endpoints[Comparing the RESTPP and GSQL query endpoints].
====
...
For a comparison of the RESTPP and GSQL endpoints for running installed queries, including differences in parameter formats and query execution, see xref:gsql-endpoints.adoc#_comparing_the_restpp_and_gsql_query_endpoints[Comparing the RESTPP and GSQL query endpoints].
...
The formatting rules for the JSON payload are the same as xref:{page-component-version}@gsql-ref:querying:query-operations.adoc#_parameter_json_object[using JSON to pass in parameters in the `RUN QUERY` command].
...
|A JSON array containing the primitive
values
|`["a", "list", "of", "args"]`
...
|`VERTEX<type>`
|A JSON object containing a field `"id"` for the
...
|List/set/bag of vertices of unspecified types
|A JSON array
...
xref:{page-component-version}@gsql-ref:querying:query-optimizer/query-profiling.adoc#_restpp_request[Query profiling] profiles detailed information about query execution for users to view and analyze.
...
[NOTE]
====
Installed queries can run in xref:{page-component-version}@gsql-ref:querying:query-operations.adoc#_detached_mode_async_option[Detached Mode].
...
In 4.3, this endpoint reports on the activity on all server nodes in a distributed cluster, not just for the node being addressed.
...
----
{
  ...
  "results": [
    ...
    {
      "requestid": "196609.RESTPP_2_1.1558386401478.N",
...
| `graph_name`
| Yes
| The name of the graph whose running queries to list.

|`disallow_redirect`
|No
|Optional flag (`true` or `false`, default `false`).
If set to `true`, the response will only include the status of queries on the current node.
|===
...
----
{
  ...
  "results": [
    {
      "requestid": "65538.RESTPP_1_1.1558386411523.N", <1>
      "startTime": "2019-05-20 14:06:51.523",
      "expirationTime": "2019-05-20 14:15:11.523", <2>
      "url": "/sleepgpe?milliseconds=100001"
    },
    {
      "requestid": "196609.RESTPP_2_1.1558386401478.N",
...
*Required privilege*: xref:user-access:rbac-row-policy/row-policy-privileges-table.adoc[Graph-level READ_DATA]
...
[source,bash]
----
curl -X GET "localhost:14240/restpp/abortquery/poc_graph?requestid=16842763.RESTPP_1_1.1561401340785.N"
----
--
Response::
+
...
| `requestid`
| No
| The ID of the query to abort. Specify either a single query ID or the string `"all"`. If `requestid` is set to `"all"`, all running queries are aborted.
...
[NOTE]
====
The `requestid` parameter accepts only a single query ID or the value `"all"`. Specifying multiple `requestid` parameters in the same request is not supported.
====
...
This endpoint allows you to check the status of a query run in xref:{page-component-version}@gsql-ref:querying:query-operations.adoc#_detached_mode_async_option[detached mode].

In 4.3, this endpoint reports on the query's status on all server nodes in a distributed cluster, not just for the node being addressed.
...
[source.wrap,bash]
----
curl -s -X GET "http://localhost:14240/restpp/query_status?requestid=4.RESTPP_1_1.1599672031541.N&requestid=4.RESTPP_2_1.1599672031541.N"
----
--
Response::
+
...
----
{
  ...
  "results": [
    ...
    {
      "requestid": "4.RESTPP_2_1.1599672031541.N",   <7>
      "startTime": "2020-09-09 10:20:31.541",        <8>
      "expirationTime": "2020-09-09 10:20:47.541",   <9>
      "url": "/query/ldbc_snb/countIndirectFriends?pid=21990232555890", <10>
      "elapsedTime": 24,                             <11>
      "status": "success"                            <12>
    }
  ]
...
----
<1> The query ID associated with the first query status JSON object.
<2> The timestamp for the start time of the first query.
<3> The timestamp for when the first query times out. 
The default timeout limit is 16 seconds and can be set using 
the xref:index.adoc#_gsql_query_timeout[GSQL-TIMEOUT] header.
<4> The URL of the first query.
<5> Elapsed real time of the first query measured in milliseconds. 
For completed queries, the value shows the total runtime of the request. 
For ongoing queries, it shows the amount of time taken so far.
<6> The status of the first query. Possible values are 
“success”, “timeout”, “aborted”, or “running”.
<7> The query ID associated with the second query status JSON object.
<8> The timestamp for the start time of the second query.
<9> The timestamp for when the second query times out. 
The default timeout limit is 16 seconds and can be set using 
the xref:index.adoc#_gsql_query_timeout[GSQL-TIMEOUT] header.
<10> The URL of the second query.
<11> Elapsed real time of the second query measured in milliseconds. 
For completed queries, the value shows the total runtime of the request. 
For ongoing queries, it shows the amount of time taken so far.
<12> The status of the second query. Possible values are 
“success”, “timeout”, “aborted”, or “running”.
--
====
...
If the query is still running, the endpoint will respond with an error and a message saying `"Unable to retrieve result for query <requestid>"`.  Ensure that the query is finished before checking its result.

In 4.3, this endpoint reports on the query results from all server nodes in a distributed cluster, not just for the node being addressed.
...
[source.wrap,bash]
----
curl -s -X GET "http://localhost:14240/restpp/query_result?requestid=196611.RESTPP_2_1.1630601692834.N"
----
--
Response::
+
...
*Required privilege*: xref:user-access:rbac-row-policy/row-policy-privileges-table.adoc[Global-level DELETE_DATA]
