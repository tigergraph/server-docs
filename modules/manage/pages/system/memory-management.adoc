= Memory Management
:page-aliases: system-management:memory-management.adoc
:description: How to monitor and manange memory usage in TigerGraph.
...
** Use with xref:cluster:cluster-commands.adoc#_run_commands_on_multiple_nodes[`grun`] to check available memory on every node.
...
You must xref:system/manage-services.adoc#_start_stop_or_restart_a_service[restart the GPE service] for the change to take effect.
