...
=== Recovering from an Incomplete Rolling Upgrade

If your rolling upgrade is interrupted or fails to complete, determine the status and resolve the issue by following the instructions below.

==== Determining the Status of Your Rolling Upgrade

To check the status of your upgrade, run the following command:
...
If you receive a clear success or error message, follow the instructions accordingly.
If the process appears stuck and you don't receive any message, it’s considered a silent failure.

==== Handling an Incomplete Rolling Upgrade

If your upgrade is incomplete, there are two options for moving forward:

. Continue with an Offline Upgrade
+
An offline upgrade requires manual intervention. To complete the upgrade, restart the services sequentially with the following command:
+
[source.wrap,bash]
----
...
----
+
Once the services are restarted, monitor the progress with this command:
+
[source.wrap,bash]
----
...
----
+
Wait until all services are online, which means the upgrade is complete.
...
[NOTE]
====
* If the issue that caused the upgrade to stop is fixed and not expected to happen again, you can try running the rolling upgrade again as usual.
* If you choose to upgrade offline or revert back to the previous version, you should halt database operations until the version change is complete.
====
