...
* `commandText` field double quotes may not be escaped correctly
** For example, the command `show function -r ".*f.*"` should be shown as `"commandText":"show function -r \".*f.*\""`, however, it is currently shown as `"commandText":"show function -r .*f.*"` for now
