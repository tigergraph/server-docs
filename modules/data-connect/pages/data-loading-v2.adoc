...
=== `HA Setup`
To enable data loading HA, use a cluster with replication factor greater than 1 following xref:cluster:ha-cluster.adoc[High Availability Cluster Configuration].

Then data loading HA will be enabled by default with data loading V2 enabled.


=== `Validation`
To confirm data loading V2 is successfully enabled, try running a loading job as usual described in xref:data-loading-overview.adoc[].
...
image::data-loading-v2-progress-example.png[Example GSQL loading progress with data loading v2 enabled]
...
== `Run Loading Job`
You can now run any loading job as usual following these instructions xref:data-loading-overview.adoc[].
