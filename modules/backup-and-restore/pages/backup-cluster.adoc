...
While database reads and writes can take place during backup, the system enforces *locking* among other cluster critical operations (  cluster resizing, `CONFIG APPLY` and upgrade). One operation will be granted the lock and others will be rejected or canceled.
