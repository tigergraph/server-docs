= Legacy Access Privileges (Deprecated)

NOTE: This privilege scheme was the one in use before
xref:manage:access/rbac-row-policy/row-policy-privileges-table.adoc[object-based privileges] were adopted.
...
It cannot be granted to a local role (See xref:manage:access/access-control-model.adoc[Global role vs local role]).
...
* As of 4.1.0, when using certain legacy query privilege syntax, a user need to config entry `GSQL.BasicConfig.Env` with `ALLOW_LEGACY_RBAC_SYNTAX` environmental variable. However, there are also several forbidden legacy RBAC syntax usages. See details and examples in xref:manage:access/fine-grained-query-privileges.adoc#_legacy_rbac_privilege_syntax_usage[Legacy RBAC Syntax Usage].


[NOTE]
====
It’s recommended to use the xref:manage:access/rbac-row-policy/row-policy-concepts.adoc#_object_based_privileges[Object-Based Privileges] syntax.
See xref:manage:access/rbac-row-policy/row-policy-privileges-table.adoc[] for a comparison with the legacy syntax.
====
...
For details see xref:manage:access/access-control-model.adoc#_data_crud_privileges[Data CRUD privileges].
|No
