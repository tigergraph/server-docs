...
To see how to Export/Inport all graphs in a database at once refer to xref:backup/database-import-export.adoc[].
...
[IMPORTANT]
====
Import/export is a complement to xref:backup/index.adoc[], but not a substitute.
====
...
It inherits  xref:backup/database-import-export.adoc#_export_all_options[Export All options] but has the following differences.
...
`IMPORT GRAPH` does not execute like `drop all` as with xref:backup/database-import-export.adoc#_export_graph_all[IMPORT GRAPH ALL] before importing specified graphs.
