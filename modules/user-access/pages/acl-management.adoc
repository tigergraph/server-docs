...
[.wrap,gsql]
----
GSQL > GRANT ACL PRIVILEGE READ ON example_query TO example_role
----


=== Revoke ACL privilege from a role
The owner of a query can revoke ACL privileges on the query from a role.
If you revoke an ACL privilege on the query from all roles, then the ACL entry status for that privilege becomes `NOBODY`.
When the ACL entry status for a privilege is unspecified, no one, including the query owner, can access the query in the corresponding manner.
