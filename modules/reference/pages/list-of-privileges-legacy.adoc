...
In version 3.10, xref:user-access:rbac-row-policy/rbac-row-policy.adoc#_object_based_privileges[Object-Based Privileges] were introduced to eventually replace the original (legacy) RBAC system.
See xref:user-access:rbac-row-policy/row-policy-privileges-table.adoc[] for a comparison with the legacy syntax.
