...
* If this is the initial installation or you are updating a previous key file, then please see the document xref:get-started:install/license.adoc[]
...
*GBAR* is the utility to do backup and restore of TigerGraph system.  Before a backup, GBAR needs to be configured. Please see xref:manage:backup/index.adoc[GBAR - Graph Backup and Restore] for details.
