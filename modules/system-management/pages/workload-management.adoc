...
[NOTE]
====
For workload management, use `username:password` for GSQL APIs and a JWT token with `-H "Authorization: Bearer <JWT-token>"` for RESTPP APIs to ensure secure and correct authentication.
====
