...
This feature maintains the same xref:backup-and-restore:configurations.adoc[] as before.
...
With online backup in place, users can seamlessly execute post requests via the xref:API:index.adoc[REST API] during the backup process.
This means that even as critical operations continue, the database is concurrently backed up, ensuring continuous data availability and preventing the inability to xref:API:upsert-rest.adoc[upsert data to the database].
====
...
Currently, the Online Backup feature has one limitation:

* If a query starts during a *full* backup, the backup will not finish until that query finishes.

However, *differential* backup does not have this limitation. Users can perform differential backups even if a query is running.
