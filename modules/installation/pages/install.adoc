...
If you install on Oracle Linux Server 8, you need to add authorized users to the `/etc/ssh/sshd_config` file to enable SSH connections to your machines on these users' accounts.

During TigerGraph installation, you'll be asked to specify a user to be the TigerGraph Linux user.
This user will have access to xref:system-management:management-with-gadmin.adoc[gadmin commands to] manage TigerGraph services.
For installation on OEL 8, it's important to allow SSH connections using the TigerGraph Linux user, along with the Linux user with superuser privileges that you are using to install TigerGraph.

. For each node, create a Linux OS user with the same username and password.
This is the TigerGraph Linux user that you'll supply during installation.
+
[source,bash]
----
...
----
+
You may skip this step, and specify a username during installation to have the installer create the user for you.
In that case, you must decide on the username now, and add it to the `sshd_config` file in the next step.
. Add the TigerGraph Linux user as well as any other user you want to allow SSH connections to the AllowUsers row in the `/etc/ssh/sshd_config` file.
+
WARNING: Make sure *all users* you want to give access to the SSH server are listed in the AllowUsers row, and ensure that you don't mistype the usernames.
If a user is not listed, no SSH connections are permitted to access their account.
#Errors could result in you losing remote access to your machine entirely.#
+
[source, bash]
----
AllowUsers <tigergrpah_linux_user> <root_user>
----
. Restart sshd.
+
[source, bash]
...
image::install-script.png[Screenshot of installation process]
...
image::gadmin-status.png[Status of all components upon successful installation]
...
[WARNING]
Deploying a database without changing the default password for the TigerGraph superuser is a security risk.
