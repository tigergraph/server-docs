...
| Vertex count
| Approximate number of vertices stored on disk in the partition. 
Note that in a running system, there may be additional in-memory data that has not yet been persisted to disk, so the actual count may be slightly higher.

| Edge count
| Approximate number of edges stored on disk in the partition.
In a running system, some edge data may still reside in memory and not yet be written to disk, so the actual count may be slightly higher.

| NumOfDeletedVertices
| Number of vertices that have been logically (soft) deleted. These vertices are marked as deleted but are not immediately removed from storage. 
Cleanup may occur only in specific cases, such as when all vertices in a segment are deleted.
