...
You can run xref:system-management:management-commands.adoc#_gadmin_config_set[`gadmin config set`] to configure their values non-interactively, or run `gadmin config entry Security.SSO.SAML` to configure their values interactively in the terminal.
...
key. You can
use @/privatekey/file/path to pass the certificate from a file.

 Before v4.1.2, this must be in PKCS#8 format (start with "`BEGIN PRIVATE KEY`"). |`+nan+`
