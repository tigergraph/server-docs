...
Refer to xref:security:encrypting-connections.adoc#_option_2_create_a_self_signed_certificate[self-signed certificates] for instructions on how to generate self-signed certificate.
...
See xref:security:index.adoc[] for more information on Security in TigerGraph.
...
=== Instructions on Enabling SSL for MirrorMaker2 (Deprecated as of 4.1.3)

Please add the following settings to the connector configuration:
...
[NOTE]
====
See xref:data-loading:load-from-kafka.adoc#_configure_the_kafka_source[Configure the Kafka Source] for more information on Kafka connection configurations.
====
