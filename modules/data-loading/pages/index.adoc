...
(xref:load-from-warehouse.adoc#_bigquery[BigQuery], xref:load-from-warehouse.adoc#_snowflake[Snowflake], and xref:load-from-warehouse.adoc#_postgresql[PostgreSql]).
...
With additional instructions on xref:avro-validation-with-kafka.adoc[Avro Data Validation through KafkaConnect] and how to xref:externalizing-kafka-configs.adoc[Externalize Kafka Configs].

[NOTE]
====
This is an improved connector for Kafka sources introduced in 4.1.3+.
The xref:load-from-kafka-mm.adoc[previous Kafka source connector, using MirrorMaker], is still supported but not recommended.
====
