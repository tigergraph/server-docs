...
* *Improved monitoring and logging* - [Alex] Facilitates better workload management for xref:3.9@:gui-docs:monitoring:queries.adoc[queries] and loading jobs
...
* Added xref:3.9@kubernetes:k8s-operator/backup-and-restore.adoc[], [Alex] version upgrade, and [Alex] resource update functionality.
...
* Added xref:1.3@pytigergraph:gds:data-:query.adoc#_runinstalledquery[NodePiece] scalable node embedding algorithm.
...
* xref:3.9@cluster-resizing:expand-a-cluster.adoc[] no longer requires a graph schema.
