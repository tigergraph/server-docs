...
The connection URL requires the domain name or IP address of the BigQuery server, the port number, the ID of your BigQuery project, and your credentials for authentication.
...
[source.wrap,text]
----
connection.url=jdbc:bigquery://<host_url>:<port_number>;<property1>=<value1>;<property2>=<value2>;...
----
...
|`host_url`
|Domain name or IP address of the BigQuery server.
Required.

|`port_number`
|Number of the TCP port to connect to.
This is usually 443.
Required.
...
** `OAuthPvtKeyPath`
* `1`: Authentication with access token or refresh token.
This requires that you supply one of the two following parameters:
** `OAuthAccessToken`
** `OAuthRefreshToken`

|`OAuthServiceAcctEmail`
|Email Google Service Account used to authenticate the connection.
...
Required if you set `AuthValue` to `1`.
|`OAuthRefreshToken`
...
NOTE: If you are querying `STRUCT` data or arrays, see
...
If you want to retrieve the entire `STRUCT`, you need to first use `TO_JSON_STRING()` to convert the `STRUCT` data into JSON strings in the converted CSV streams, and then xref:gsql-ref:ddl-and-loading:creating-a-loading-job.adoc#_flatten_a_json_object_in_a_csv_file[flatten the JSON strings] when loading the data into the graph.
