...
From *3.9.3*, when xref:reference:configuration-parameters.adoc#_controller[`Controller.ServiceManager.AutoRestart`] is set to `true`, TigerGraph automatically restarts all services if they crash during `gadmin start` or `gadmin restart`.
