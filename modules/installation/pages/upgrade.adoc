...
Now that it is added as a built-in function, it is no longer needed in the `ExprFunctions` file.
...
Users can either rename, remove, or comment out any functions in their `ExprFunctions` file that contain this prefix and avoid prefixing future functions with this reserved prefix.
...
Additionally, user can also disable the file policy after upgrade, so the restrictions won’t take effect.

=== Input/output file policy

Users to either modify existing queries/loading jobs, or modify file input/output policy after upgrade.
