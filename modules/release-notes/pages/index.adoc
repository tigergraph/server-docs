...
** [3.9.3] Added new REST parameter `/allowExisting` for token endpoints (GLE-5439)
...
** [3.9.3] Additional commands were added to support user determined compression levels of backups and restores. (TP-2564)
** [3.9.3] Added two new commands that allowed CPU usage based GPE query routing via a user determined threshold (CORE-2429)
...
* [3.9.3] xref:tigergraph-server:backup-and-restore:cross-cluster-backup.adoc[Simplified cross-cluster restore] process by adding metadata to backup files and still supporting the previous method for older versions. (TP-3616)
...
* [3.9.3] Added `_max_percent_error` command for user configurability when validating loading data inputs
...
==== Schema
* [3.9.3] xref:tigergraph-server:user-access:user-management.adoc[Reduction of GSQL username character restrictions] for more flexible username creation for v3.9.3 and future versions (GLE-5545).
...
* [3.9.3] Added openCypher syntax support to xref:gui:graphstudio:write-open-cypher-queries-in-tigergraph.adoc[].
* [3.9.3] Enabled the "Share Insights" to share Insights widgets without needing a login.
...
* Fixed issue where administrator could privilege of vertex `primary_id` in Role Management page of Admin Portal (TOOLS-2430).
* Fixed issue where error messages of abort requests created large file sizes (CORE-2705).
* Fixed issue where GPE was still in warmup while running chaos testing (CORE-2670).
...
* Fixed issue where delete or edit vertex attributes were incorrect in GST (TOOLS-2258).
...
==== Improvements
* Improved Insights table widget to be more consistent with graph studio (TOOLS-2375).
...
* Improved precision of `INT` and `UNIT` (CORE-2703).
* Improved error message details when during a failure to find a request record when deleting (CORE-2686).
* Improved API user existent check for when permission is not needed (GLE-5678).
* Improved performance of edit vertex when vertex 200 attributes. (TOOLS-2107)
* Added memory check in `DistrbutedTopologyStore` (CORE-2496).
* LDAP now has more detailed error messages (GLE-5559).
* Implemented connector fail status to show error messages in GSQL console (TP-3465).
* Added link to TigerGraph Kubernetes preview documentation on GitHub (TP-4039).
* Expended support for `gcollect` (TP-4127).
* Improved Insights tables by adding overflow setting to tables with many column (TOOLS-2312).
...
== Known issues
* An exception error is thrown when "" fail to covert to int64 when using `gcollect` to collect the tsar data.
* After a global loading job is running for a while a fail can be encountered when getting the loading status due to `KAFKASTRM-LL` not being online, when actually the status is online. Then the global loading process will exit and and fail the local job after timeout while waiting the global loading job to finish.
* For v3.9.1 and v3.9.2 when inserting a new edge in `GPR` and `INTERPRET` mode, the GPE will print out a warning message, because it calls `graphupdates()` and `getau()` API and the `subtypeid` and discriminator string is not set for new-inserted edges.
Creating an inconsistent problem in delta message for GPR and `INTERPRET` mode.
