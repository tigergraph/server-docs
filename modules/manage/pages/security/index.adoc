...
* xref:security/encrypting-connections.adoc[Connection encryption (TLS 1.2)]

== Authentication

* xref:access/enabling-user-authentication.adoc#_enable_restpp_authentication[RESTPP authentication]
* xref:access/enabling-user-authentication.adoc#_enable_gsql_authentication[GSQL authentication]
* xref:access/sso.adoc[SSO with SAML 2.0]
** xref:access/sso.adoc#_azure_ad[Azure AD]
** xref:access/sso.adoc#_okta[Okta]
** xref:access/sso.adoc#_auth0[Auth0]
** xref:access/sso.adoc#_pingfederate[PingFederate]
* xref:access/ldap.adoc[LDAP authentication]
* xref:security/password-policy.adoc[Strong password policy enforcement](TigerGraph 3.7+)

== Authorization

* xref:access/access-control-model.adoc#_role_based_access_control[Role-based Access Control (RBAC)]
* Audit logs for privileged user actions

== Data encryption
* xref:security/encrypting-connections.adoc[Data in-transit encryption (TLS 1.2)]
* xref:security/encrypting-data-at-rest.adoc[Data at-rest encryption]
* <<_pgp_key,PGP key-signed download package>>
* xref:data-connect:kafka-ssl-security-guide.adoc[]
