...
This env config is only needed when the URL fails with `Problem with the SSL CA cert (path? access rights?)` or a related error.

For all potential JWT errors see the section xref:tigergraph-server:user-access:jwt-token.adoc#_potential_error_messages[potential error messages].

The storage location of the CA certificate also may differ with the operating system being used.

* In Ubuntu, as a single file (PEM bundle) in `/etc/ssl/certs/ca-certificates.crt`.
...
If the `SSL_CA_CERT` is not set the default value is: `default value`.
