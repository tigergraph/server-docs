...
* Added optional `async` parameter to **xref:tigergraph-server:API:built-in-endpoints.adoc#_install_a_query[install a query]** in the background.

* Added ability to **specific a workload queue** when **xref:4.1@tigergraph-server:system-management:workload-management.adoc#_running_a_query[running an query]** or **xref:4.1@gsql-ref:querying:query-operations.adoc##interpret-a-created-query[interpreting a created query]**.
