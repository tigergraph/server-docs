...
If the system was handling a user request, the status code and message will be in the JSON response (see xref:{page-component-version}@gsql-ref:querying:query-operations.adoc#_gsql_query_output_format[GSQL Query Output Format] ).
...
| `REST-1005`
| Query endpoint is disabled. Make sure all its sub-queries are installed and enabled with the same signature.
|===
...
| `REST-13000`
| RESTPP failed to communicate with a remote node while forwarding a request. This usually indicates a node-to-node communication failure.
...
| `REST-30200`
| The parameter for xref:API:upsert-rest.adoc[upserting data] is invalid.

| `REST-30400`
| The parameter for xref:API:built-in-endpoints.adoc#_show_query_performance[showing query performance] is invalid.
|===
