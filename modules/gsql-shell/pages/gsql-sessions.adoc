...
| `profile` | This parameter enables xref:{page-component-version}@gsql-ref:querying:query-optimizer/query-profiling.adoc#_gsql[query profiling] to view and analyze the details of a query execution. It currently supports only the value `BASIC`. By default, the value is empty, and profiling is not activated.
