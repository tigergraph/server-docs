...
The default value of `Security.JWT.UserClaim` is `sub`. Users can configure this parameter to specify which JWT payload claim TigerGraph uses to resolve the TigerGraph username. If an administrator sets the value to empty, TigerGraph falls back to using the `sub` claim.
