...
This endpoint returns real-time query performance statistics over the given time period, as specified by the `seconds` parameter.
The `seconds` parameter must be a positive integer less than or equal to 60.
...
{
    "secret": <secret_value>, <1>
    "lifetime": <token_lifetime> <2>
}
----
<1> User's secret to generate the token.
Required  if username and userpassword are not suplied.
<2> Period of time for which the token is valid measured in seconds. The default value is about 2.6 million (about a month).
