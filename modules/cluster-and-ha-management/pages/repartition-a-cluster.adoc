= Cluster Repartition
//:page-aliases: cluster-resizing:repartition-a-cluster.adoc"
...
During cluster repartition, the system enforces *locking* among other cluster-critical operations (including backup/restore, `CONFIG APPLY`, and upgrades). Only one such operation can hold the lock at a time; other operations will be rejected or canceled.
