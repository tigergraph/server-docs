...
- Name: testUser
    ...
    - Secret: 7au****gs5
      - Alias: AUTO_GENERATED_ALIAS_he24bbo
      - CreatedAt: 2026-07-23 00:04:35
...
Proxy users represent users authenticated through Single Sign-On (SSO).
They do not have local passwords and authenticate only through SSO.

The `SHOW PROXY USER` command displays metadata about the given proxy user(s).

=== Syntax

[source,gsql]
----
SHOW PROXY USER <proxy_user_name>
----

=== Required privilege
`READ_USER` (unless showing the current user)

=== Example

[source,gsql]
----
GSQL > SHOW PROXY USER Alice
----
...
[NOTE]
====
Proxy group membership is determined during SSO login based on identity provider attributes.
...
SHOW PRIVILEGE ON USER <proxygroup> (, <proxygroup>)*
----
...
. From the GSQL shell, run the `SHOW PRIVILEGE ON USER` command with either a username or proxy group name:
+
[source,gsql]
----
...
[source,gsql]
----
GSQL > SHOW PRIVILEGE ON USER proxy_group1
----

The command displays the privileges assigned to the specified user or proxy group.
...
When you run the command for a proxy group, it displays the privileges granted to that proxy group.

[source,text]
----
ProxyGroup: "proxy_group1"
  - Global Privileges:
    READ_DATA
    WRITE_DATA
    READ_USER
...
== Grant a role to a user or proxy group
...
== Grant default roles to a user

Use `GRANT DEFAULT` to add one or more users to the default list for a specific query-level privilege.
...
. The following command grants default `READ` privileges on all newly created queries in a graph to a specified user:
+
[source,gsql]
----
GSQL > GRANT DEFAULT READ ON ALL QUERIES IN GRAPH MyGraph TO USER john_smith
----
This allows `john_smith` to automatically receive query-level `READ` access on any new query created in `MyGraph`, without requiring additional `GRANT` commands.

== Revoke default roles from a user

Use `REVOKE DEFAULT` to remove a user from the default list for a specific query-level privilege.
...
. The following command revokes default `READ` privileges on all newly created queries in a graph from a specified user:
+
[source,gsql]
----
GSQL > REVOKE DEFAULT READ ON ALL QUERIES IN GRAPH MyGraph FROM USER john_smith
----
This prevents `john_smith` from automatically receiving query-level `READ` access on any new query created in `MyGraph`. Existing queries where `john_smith` already has `READ` remain unchanged.
