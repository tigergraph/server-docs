...
[CAUTION]
====
Please be aware to pass parameters in the json payload for `/requesttoken` api from 3.5. If the old API version is used, it could cause information leakage.
====
...
curl -X POST http://localhost:9000/requesttoken \
     -d '{"secret": "your_secret_value", "lifetime": "100000"}' 

curl --user example_username:example_password \
     -X GET http://localhost:9000/requesttoken \
...
[WARNING]
====
Treat your tokens like passwords and keep them secret. Tigergraph Support cannot restore access to tokens for security reasons. Please save your token and keep it safe and accessible.
====
...
curl -X PUT http://localhost:9000/requesttoken \
     -d '{"secret": "your_secret_value", "token": "your_token_value", "lifetime": "15000"}' 

curl --user example_username:example_password \
     -X PUT http://localhost:9000/requesttoken \
     -d '{"token": "your_token_value", "lifetime": "15000"}'
...
| `secret`
| Yes if username and userpassword are not suplied
...
curl -X DELETE http://localhost:9000/requesttoken \
     -d '{"secret": "your_secret_value", "token": "your_token_value"}' 
...
| `secret`
| Yes if username and userpassword are not supplied
| User's secret used to generate the token.
|===
