...
+
Once the service is up and running, test it, by making an update to an existing graph with xref:3.9@gsql-ref:querying:data-modification-statements.adoc[].
+
Statements like:
+
* xref:3.9@gsql-ref:querying:data-modification-statements.adoc#_update_statement[Update]
* Running a custom or xref:3.9@gsql-ref:tutorials:gsql-101/built-in-select-queries.adoc[built-in query]
* Running a xref:tigergraph-server:API:built-in-endpoints.adoc#_loading_jobs[loading job].
+
[NOTE]
====
If an existing graph is not available, create a new graph by following TigerGraph’s xref:3.9@gsql-ref:tutorials:gsql-101/index.adoc[] tutorial documentation and using the provided xref:3.9@gsql-ref:appendix:example-graphs.adoc[] data.
====
+
