...
== Versioning
TigerGraph uses https://semver.org/[semantic versioning]. Versions are numbered in the format `MAJOR.MINOR.PATCH`:

* `MAJOR` version releases indicate API changes to the previous major version resulting in incompatibility
* `MINOR` version releases indicate added functionality in a backwards compatible manner
* `PATCH` version releases indicate backwards compatible bug fixes.
