...
----
+
.The prefix of GPE/GUI/GSQL Kafka Topic, by default is empty. For setting up new DR after failover, refer to xref:tigergraph-server:cluster-and-ha-management:fail-over.adoc#_set_up_a_new_dr_cluster_after_failover[set up a new dr cluster after failover].
[source.wrap,console]
----
