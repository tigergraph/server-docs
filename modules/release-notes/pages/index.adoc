...
* [4.1.1] **xref:manage:cdc/cdc-message-example.adoc#_other_message_fields[CDC messages have a "graph" field]** to specify which graph was affected.

* [4.1.1] **xref:cluster:crr-index.adoc[Cross-Region Replication]** now includes loading jobs.
...
* Added **xref:4.1@gsql-ref:querying:query-operations.adoc#_detached_mode_async_option[Detached mode -async]** and **xref:4.1@tigergraph-server:system-management:workload-management.adoc#_running_a_query[Workload Queue]** option for runOptions when **xref:4.1@gsql-ref:querying:query-operations.adoc##interpret-a-created-query[interpreting a created queries]**.
