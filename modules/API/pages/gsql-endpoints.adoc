...
[source,bash]
----
curl -X POST -H "content-type: text/plain" -H 'Workload-Queue: <your_workload-query_name>' 'localhost:14240/gsql/v1/queries/interpret?p1=hello&p1=world' -d 'INTERPRET QUERY (SET<STRING> p1) FOR GRAPH financialGraph syntax v1 {  print p1; }'

//can add two headers?one for Content-Type: text/plain and one for Workload-Queue: <Workload Queue Name>? 
----
...
[source,bash]
----
curl -X POST -H "content-type: text/plain" "http://localhost:14240/gsql/v1/queries/interpret?p1=hello&p1=world&queue=<your_worload_queue_name>" -d 'INTERPRET QUERY -QUEUE queue (SET<STRING> p1) FOR GRAPH financialGraph syntax v1 {  print p1; }' 
----
