...
* **xref:3.11@tigergraph-server:backup-and-restore:configurations.adoc#_configuration_parameters[Role ARN for Backup to AWS S3 buckets]**:
...
* xref:3.11@tigergraph-server:system-management:change-data-capture/cdc-message-example.adoc#_extra_cdc_message_for_edge_update[Change Data Capture] (CDC) now generates `insert-only` messages for newly created source and target vertices during edge insertion when `primary_id_as_attribute="true"`, providing more complete CDC event tracking.

* Added support for the xref:3.11@tigergraph-server:API:built-in-endpoints.adoc#_parameters_13[`header` parameter] in the `/ddl` endpoint, allowing loading jobs to skip the first line of input data when it contains column headers.
...
* Added the ability to set xref:3.11@gsql-ref:querying:data-types.adoc#_set_s3_connection_credentials[AWS S3 access credentials] as GSQL session parameters, enabling users to configure S3 query output without requiring admin privileges.
...
+
Fixed the following security vulnerabilities: CVE-2023-44981, CVE-2024-43382, CVE-2024-8184, CWE-311, CWE-400, and CWE-639.
+
...
¦ When xref:{page-component-version}@installation:upgrade.adoc#_upgrading_from_3_x[upgrading], possible permission error for destination folder.
¦ 3.10.1
...
¦ The use of plaintext tokens in xref:3.11@tigergraph-server:API:authentication.adoc[authentication] is deprecated.
Use xref:3.11@tigergraph-server:user-access:jwt-token.adoc[] instead.
¦ 3.10.0
¦ 4.1
...
If you are using a version of TigerGraph before 3.10.0, you can use `gbar` to create a backup file and restore it in 3.11 with `gadmin backup restore`.

¦ 3.7
¦ 3.10.0

¦ xref:3.11@tigergraph-server:user-access:vlac.adoc[Vertex-level Access Control (VLAC)] and xref:3.11@gsql-ref:querying:func/vertex-methods.adoc#_vlac_vertex_alias_methods_deprecated[VLAC Methods] are now deprecated and will no longer be supported.
¦ 3.10.0
¦ 4.0

¦ xref:3.11@tigergraph-server:data-loading:spark-connection-via-jdbc-driver.adoc[Spark Connection via JDBC Driver] is now deprecated and will no longer be supported.
¦ 3.10.0
¦ TBD
