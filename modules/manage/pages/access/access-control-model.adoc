...
Authentication is not enabled by default and xref:access/enabling-user-authentication.adoc[must be enabled manually].
...
[NOTE]
====
For Row Policy related Built-in roles see xref:access/rbac-row-policy/row-policy-concepts.adoc#_built-in-roles[Row Policy built-in role changes]
====

=== User-defined roles

Users can define roles with their own list of privileges they want to grant to the role.
To learn how to create/drop user-defined roles and manage privileges for the roles, see xref:access/role-management.adoc[Role Management].
