...
¦ `tokenBankException` ¦ Exception thrown by token bank functions. This is a generic error type for failures that occur during the execution of user-defined token bank functions. The exception does not specify which function failed or the cause, as it serves to classify and count such errors for monitoring and debugging purposes.
|===
