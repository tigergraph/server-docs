...
Other brower-based products, such as TigerGraph Insights, have their own browser requirements. See their corresponding documentation.
...
The sizing recommendations below apply to each server node. If you have more than a several hundred GBs of data, you should consider deploying a cluster of multiple nodes, to distribute your data.
...
This section descriptions considerations beyond sizing for the CPU, memory, and storage.
...
RAM is not only size but also type. DDR4 is recommended because it gives you significantly faster data transfer rates, better power efficiency, and better data granularity than DDR3.

==== Disk

As stated in the <<_hardware_requirements>> section, solid state drive (SSD) storage is *mandatory* for TigerGraph.
The longer data access times of spinning optical disks (HDD) can cause several issues, such as the following:
...
```
TIP: To economize, you can opt to use an optical disk (HDD) for the logs; the remaining ones must be SSD.
