...
The TigerGraph system supports LDAP authentication by allowing a TigerGraph user to log in using an LDAP username and credentials.
During the authentication process, the GSQL server connects to the LDAP server and requests the LDAP server to authenticate the user.

This page provides instructions on how to configure LDAP, map LDAP users to GSQL users, as well as specific instructions for integration with Azure Active Directory (AD).
...
If you are using Azure AD, set this parameter to `sAMAccountName`.
...
* Set the value to `none` if you don't want to use any encryption protocol.
