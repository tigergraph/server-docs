...
| The template to generate nginx config. Please use `@filepath` to parse template from file. Check the default template first at https://docs.tigergraph.com.(Warning[https://docs.tigergraph.com.(Warning]: Don't modify the reserved keywords(string like *UPPER_CASE*) in template.)
