...
You can also request a gzip-compressed response when running a query from the GSQL shell , see xref:gsql-ref:querying:query-operations.adoc#_gzip_compression_gzip_option[`-gzip` option] of the `RUN QUERY` command.
