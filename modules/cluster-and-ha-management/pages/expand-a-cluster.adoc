= Cluster Expansion
//:page-aliases: cluster-resizing:expand-a-cluster.adoc
:sectnums:
...
During cluster expansion, the system enforces *locking* among other cluster-critical operations (including backup/restore, `CONFIG APPLY`, and upgrades). Only one such operation can hold the lock at a time; other operations will be rejected or canceled.
