...
During a rolling upgrade, the following operations are currently blocked:
...
Additionally, please avoid the following operations, as they will interfere with or be affected by the rolling upgrade process:

  * backup and restore
  * cluster reszie (expansion, shrink, or removal)
  * configraion change via `gadmin config set/apply`
  * Starting or stopping services via `gadmin start/stop/restart`
  * Setting up CRR (note: CRR will continue to function if it is already configured)
  * cluster initialization via `gadmin init`
  * reset service via `gadmin reset`
...
=== Failure Handling During Rolling Upgrade

==== Resume Rolling Upgrade

During the rolling upgrade, if the connection is lost unexpectedly, you can reconnect to the upgrade by running:
...
==== Rolling Upgrade failed with error

If the rolling upgrade encounters an issue, the error message will guide you on how to continue the upgrade or manually revert to the previous version. Here is an example of the error prompt:
...
An unexpected restart of the CTRL orchestrating the rolling upgrade will result in a silent failure. In this case, both the upgrade status and the upgrade connection will be lost, as if no rolling upgrade had ever been triggered.

Subsequent attempts to reconnect using `gadmin upgrade --resume` will result in the following message:
[source.wrap,bash]
----
$ gadmin upgrade --resume
[   Info] no rolling upgrade is underway
----
In such cases, you may manually switch to the new version or revert to the old version.

[IMPORTANT]
====
These steps will cause downtime.
====

===== Switch to the New Version

----
# Set System.AppRoot to the new version (e.g., 4.2.1)
...
----
# Set System.AppRoot to the old version (e.g., 4.2.0)
