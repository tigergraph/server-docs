= Cluster Shrinking
//:page-aliases: cluster-resizing:shrink-a-cluster.adoc
:sectnums:
...
During cluster shrinking, the system enforces *locking* among other cluster-critical operations (including backup/restore, `CONFIG APPLY`, and upgrades). Only one such operation can hold the lock at a time; other operations will be rejected or canceled.
