...
[NOTE]
====
This endpoint and the xref:api:gsql-endpoints.adoc#_run_query[GSQL Run query] endpoint perform the same core function of running an installed query.

TigerGraph provides the GSQL endpoint as part of a standardized GSQL API. If your application already uses the RESTPP API, you can continue using this endpoint.
====
