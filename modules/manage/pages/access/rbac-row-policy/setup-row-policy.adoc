...
This guide assumes some basic understanding of xref:access/rbac-row-policy/row-policy-concepts.adoc[].

Additionally, before going through the examples below it's a good idea to read through creating a user and assigning roles in our current documentation on xref:access/user-management.adoc[] and xref:access/role-management.adoc[].
...
* Use xref:access/rbac-row-policy/row-policy-concepts.adoc#_gsql_functions[GSQL Functions] to manage these fine-grained controls and permissions.
...
For more information on REST API and REST API endpoints see the documentation on xref:reference:REST/index.adoc[] and xref:reference:REST/built-in-endpoints.adoc[].
