...
¦ When you insert a directed edge without a reverse edge type, TigerGraph automatically creates the source and/or target vertex if they don’t exist.

Extra CDC messages are generated:
...
When Undirected Edge or Directed Edge With a Reverse Edge Type, TigerGraph automatically creates the source and/or target vertex if they don’t exist:
