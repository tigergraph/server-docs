...
A `RUN LOADING JOB` command may only use one type of data source: you cannot mix both Kafka data sources and regular file data sources in one loading job.
...
If a loading job is interrupted, you need to manually resume the loading job.
