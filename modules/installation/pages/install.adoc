...
WARNING: Since license keys are long (over 100 characters), if you copy-and-paste the license key, be careful not to accidentally include an end-of-line character.
...
After installation is complete, you can switch to the Linux user who owns the platform (created in link:install.adoc#_step_2_run_installation_script[Step 2]) with the following command :
...
* The default root directory for the installation would be `/home/tigergraph/tigergraph` with the application, data, log and temporary files within it:
