...
[NOTE]
====
For a complete list of Built-in roles see xref:user-access:access-control-model.adoc#_built_in_roles[Built-in Roles]
====
...
Additionally, in order to use the context functions explicitly, ensure that the user holds the `READ_ROLE` privilege on the current graph, unless a xref:user-access:rbac-row-policy/setup-row-policy.adoc#_row_policy[Row Policy] already includes the context functions.
