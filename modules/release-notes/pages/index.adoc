...
* Fixed known issue where the attribute name `memberOf` was case-sensitive. It is now case-insensitive.
...
¦When using xref:tigergraph-server:backup-and-restore:database-import-export.adoc[Import All] if a users `.zip` file  is exceedingly large, the import may fail with an error messages like this:
