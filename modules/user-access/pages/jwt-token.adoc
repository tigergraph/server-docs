...
¦ `aud`
¦ string
¦ [optional] Audience name of the JWT Token.
...
Users can set this JWT Token authentication to verify if the `aud` defined in the JWT Token matches the configured one or not.
It will fail the verification process if they don’t match.
