...
During a rolling upgrade, expect temporary service interruptions. If the upgrade process fails or the connection is lost unexpectedly, you can resume the upgrade by running:
...
If the rolling upgrade encounters an error, the error message will guide you on how to proceed. You can manually finish the upgrade by following these steps in order:
...
Sometimes the rolling upgrade may fail without showing any error, like if the controller node is restarted manually. 
When this happens, you can either *manually switch to the new version* yourself or *revert to the previous version*.
