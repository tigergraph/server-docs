...
=== Upserting Multi-Edges with Discriminators
...
=== JSON Format for Upserting Discriminated Edges

This format for upserting multi-edges with discriminators is still supported for backward compatibility. 
You can continue using this format if preferred, however the JSON array format offers greater flexibility for inserting multiple edges with different discriminators in a single API request.
