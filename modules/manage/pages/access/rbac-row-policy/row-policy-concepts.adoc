...
:page-aliases: rbac-row-policy.adoc
...
To see a complete list, as well as the xref:reference:list-of-privileges.adoc[legacy privilege syntax] that the object-base privilege relate to, go to the xref:access/rbac-row-policy/row-policy-privileges-table.adoc[].
...
[NOTE]
====
For a complete list of Built-in roles see xref:access/access-control-model.adoc#_built_in_roles[Built-in Roles]
====
...
To learn more about REST++ authentication see xref:reference:REST/rest-authentication.adoc[REST API Authentication].

Additionally, in order to use the context functions explicitly, ensure that the user holds the `READ_ROLE` privilege on the current graph, unless a xref:access/rbac-row-policy/setup-row-policy.adoc#_row_policy[Row Policy] already includes the context functions.
