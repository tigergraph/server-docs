...
//:page-aliases: ha:remove-failed-node.adoc
:description: This page describes the procedure to remove a failed node.

If a node in a high availability (HA) cluster has an unrecoverable failure, such as a hardware failure, you can remove the failed node from the cluster while keeping all your data intact.
After removal, use the xref:expand-a-cluster.adoc[cluster expansion] feature to maintain the remaining nodes in a functioning state.
