...
In stream mode, an end-of-file character causes the loader to pull all of the files in the target directory again and check when they were last edited.
Any files that were modified more recently than the files already in the data loading queue are loaded again.
...
For example, consider a file `test.txt` in cloud storage that is part of a loading job.
...
Lines 1 and 2 are loaded successfully into the loading job.
If a user edits the file and adds a new line, `line-3`, the stream loader notices the new modification and adds the file to the loading queue again.
Lines that have already been loaded are skipped.
...
If a user edits the file and deletes or modifies a line before the end, like so, the entire file is loaded again, causing potentially repeated data.
