...
[NOTE]
====
In addition, users can utilize external sources to provide configurations for Kafka connectors. This includes files, Vault, and environment variables. See xref:tigergraph-server:data-loading:externalizing-kafka-configs.adoc[].
====
...
Required for third party `<prefix>.sasl.mechanism` configurations.
...
The following configurations are required for admin, producer and consumer. Kafka allows SSL settings overriding, respecting security settings in precedence order:

* `generic.ssl.setting` < `source/target.cluster.ssl.setting` < `admin/producer/consumer.ssl.setting`.

If both source and target clusters are sharing the same SSL settings, user can set generic settings for both source/target clusters and all the rols(admin/producer/consumer). For example, user can set `ssl.keystore.location="/path/to/key/store"` instead of:
...
If source and target clusters have different SSL settings, it is possible to set cluster wide SSL configs. For example: `target.cluster.ssl.truststore.password="/password/for/trust/store"`, instead of `target.cluster.producer.ssl.trust.password="/password/for/trust/store"`.
...
----
{
  ...
  "consumer.sasl.client.callback.handler.class": "optional, need to put third party jar under `$(gadmin config get System.Approot)/kafka/libs/`",
  ...
  "source.admin.sasl.client.callback.handler.class": "optional, need to put third party jar under `$(gadmin config get System.Approot)/kafka/libs/`",

  "producer.security.protocol": "SASL_PLAINTEXT",
  "producer.sasl.mechanism": "GSSAPI/PLAIN/etc.",
  "producer.sasl.jaas.config": "<jaas_conf_content>",
  "producer.sasl.client.callback.handler.class": "optional, need to put third party jar under `$(gadmin config get System.Approot)/kafka/libs/`"
}
----
