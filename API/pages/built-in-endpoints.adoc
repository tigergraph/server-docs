...
The endpoints in this subsection allow users to create, refresh and delete authentication tokens for requests made to the REST{pp} server. *These endpoints only exist when* link:../../admin/admin-guide/user-access/user-privileges-and-authentication.md#rest-authentication[*user authentication is enabled*] *on RESTPP endpoints.*
...
* A secret, which is a random string generated in GSQL (seexref:../../admin/admin-guide/user-access/user-privileges-and-authentication.adoc[Managing User Privileges and Authentication])
...
| `graph`
| No
| Name of the graph that the token wil be valid for.
|===
...
The maximum size of data you can upload via this endpoint is controlled by the link:intro.md#request-body-size[`Nginx.ClientMaxBodySize`] configuration parameter (default is 128 MB).
  ...
  +++<tbody>++++++<tr>++++++<td style="text-align:left">++++++<code>+++ack+++</code>++++++</td>+++
    ...
    +++<tr>++++++<td style="text-align:left">++++++<code>+++vertex_must_exist+++</code>++++++</td>+++
      ...
      +++<td style="text-align:left">+++Boolean value that indicates whether or not to insert new vertices. If
        the value is true, the operation will only update existing vertices+++</td>++++++</tr>++++++</tbody>++++++</table>+++
...
{% tabs %}
{% tab title="Upsert Example Data 2: add_id6.json" %}
...
When using a `GET` request to run an installed query, the query parameters are link:intro.md#query-string-parameter-passing[passed in through the query string of the URL].
...
When using a `POST` request to run an installed query, the query parameters are passed in through the request body and link:intro.md#formatting-data-in-json[encoded in JSON format].
