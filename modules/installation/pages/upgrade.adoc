...
:description: Instructions for upgrading an existing TigerGraph installation. 
...
* xref:backup-and-restore:backup-and-restore.adoc[Use GBAR to perform a backup *and restore*] on your existing installation.
This step is necessary because TigerGraph {page-component-version} made major changes to the Graph Storage Engine and a restore is needed to remove certain files that would make the upgrade operation fail.
