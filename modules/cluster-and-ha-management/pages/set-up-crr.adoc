...
//:page-aliases: crr:set-up-crr.adoc
:description: Instruction on how to set up the DR cluster for cross-region replication.
:sectnums:

To set up cross-region replication (CRR), you set up Disaster Recovery (DR) clusters that synchronize with the primary cluster.
...
* The DR cluster and the primary cluster are running the *same version of TigerGraph (3.10 or higher)*.
* The DR cluster and the primary cluster have the *same partitioning factor*.
* You have *access to the TigerGraph Linux user account on the DR cluster*.
** The *username and password* of the TigerGraph database user creates on the DR cluster during installation *matches one of the superusers on the primary cluster*.
* Initializing the DR cluster requires running a restore operation from a recent backup of the primary cluster; therefore, *xref:backup-and-restore:configurations.adoc[backup and restore must be configured] on the DR cluster*.
...
For more details refer to xref:backup-and-restore:backup-cluster.adoc[].

For how to restore a cluster from a backup that was created from another database cluster (cross cluster), refer to xref:backup-and-restore:cross-cluster-backup.adoc[].
...
For special cases, such as enabling Cross-Region Replication (CRR) for a new cluster, you can omit the backup.

However, this applies only when setting up a new DR cluster in an empty state. Both the Primary and DR clusters must not contain any existing data (for example, after a fresh installation or after running `DROP ALL`).

The command `gadmin backup restore --dr` does not overwrite existing data in the cluster.

[WARNING]
====
Do not use this command on a cluster with existing data. It will not overwrite existing data and may lead to an inconsistent state.
====
...
Specifically, when switching between PR and DR, before the switch, it is necessary to xref:cluster-and-ha-management:troubleshooting.adoc#_check_data_consistency_between_primary_and_dr[check data consistency between primary and DR]
...
== Cross-Region Replication topology limitations

Cross-Region Replication (CRR) supports multiple Disaster Recovery (DR) clusters replicating from a single Primary (PR) cluster. However, chained replication is not supported.

=== Supported topology

A single Primary cluster can replicate to multiple DR clusters in parallel.
...
In this configuration, each DR cluster independently receives replication data directly from the Primary cluster.

=== Unsupported topology (chained replication)

Chained replication, where a DR cluster serves as the replication source for another DR cluster, is not supported.
...
A DR cluster cannot act as the source of replication for another DR cluster.

=== Performance considerations

Each DR cluster consumes replication data directly from the Primary cluster. As the number of DR clusters increases, the load on the Primary cluster also increases.

Factors that can affect replication performance include:

* Network egress bandwidth
* Disk I/O and CPU utilization on the Primary cluster
* Kafka connection overhead
* Cross-region network latency and bandwidth

The maximum number of supported DR clusters depends on workload characteristics, hardware resources, network capacity, and replication volume.

In most deployments, two to three DR clusters can replicate from the same Primary cluster without significant replication delay.
...
** RESTPP endpoints that modify graph data:
*** Run loading job 
*** Upsert operations for vertices or edges
*** Delete operations for vertices and edges
* *xref:pytigergraph:core-functions:index.adoc[pyTigerGraph functions]* that modify data or metadata
...
. xref:installation:upgrade.adoc[Upgrade] the cluster.
