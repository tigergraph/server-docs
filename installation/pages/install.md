...
{% hint style="info" %}
If you signed up for the Enterprise Free license, you also have access to the TigerGraph platform as a Docker image or a virtual machine \(VirtualBox\) image.   Follow the instructions in [Getting started](../../../cloud/getting-started.md) to start up TigerGraph in a Docker container or with VirtualBox.
{% endhint %}
...
* One or more machines that meet the minimum [_Hardware and Software Requirements_](../hw-and-sw-requirements.md).
...
* A TigerGraph system package
  * If you do not yet have a TigerGraph system package, you can request one at the following address: [https://www.tigergraph.com/get-tigergraph](https://www.tigergraph.com/get-tigergraph/)
...
{% tabs %}
{% tab title="Example: extract for <version> = x.y.z" %}
```bash
...
{% tabs %}
{% tab title="Example: Default installation for <version> = 3.1.0" %}
```bash
...
To see what the default settings are, read the [Installation options](install.md#installation-options) section below.
...
After installation is complete, you can switch to the Linux user who owns the platform \(created in [Step 2](install.md#step-2-run-installation-script)\) with the following command :
...
* `"AdvancedConfig"`
  * `"ClusterConfig"`: Cluster configurations
    ...
    * `"ReplicationFactor"`: [Replication factor](../../../tigergraph-platform-overview/continuous-availability-overview.md#continuous-availability-definitions) of the cluster. 
