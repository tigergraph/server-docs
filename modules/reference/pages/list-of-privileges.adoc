...
* To run the command `CREATE SECRET` on a graph, the user must have at least one of the access database privileges, `READ_DATA`, `CREATE_DATA`, `UPDATE_DATA`, `DELETE_DATA` or `EXECUTE_LOADINGJOB`, on that graph. Thus the built-in `queryreader` role and above can create secrets on a graph, but the observer role cannot.
