...
TigerGraph Server {page-component-version}.0 was released on June 17th, 2022.
...
=== Fixed in 3.6.2
TigerGraph Server 3.6.2 was released on August 10th, 2022.
...
* Fixed an issue where S3 loaders created in GraphSudio do not self-delete after loading sample data.
...
* Fixed an issue where the built-in `searchvertex` query could cause GPE dysfunction in rare cases.
* Increased the restrictiveness of file permissions for snapshot files.
* Fixed an issue where requests to GSQL endpoints would return error if the request also includes setting cookies.
