...
. Attend to security.
+
WARNING: Your TigerGraph image is preconfigured with a superuser having the default username and password `tigergraph`. If you do not change this, anyone with access to your database will be able to read and modify it.
+
To xref:tigergraph-server:user-access:user-management.adoc#_change_a_users_password[change the password] of the user called `tigergraph`:
+
[source,console]
----
...
----
+
For additional security settings, see xref:tigergraph-server:security:index.adoc[].
