...
While restoring a backup, the system enforces *locking* among other cluster-critical operations (cluster resizing, `CONFIG APPLY`, and upgrades). Only one such operation can hold the lock at a time; other operations will be rejected or canceled.
