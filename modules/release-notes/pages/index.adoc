...
* [4.2.1] *xref:4.2@tigergraph-server:API:built-in-endpoints.adoc[RESTPP endpoints `/query_status`, `/query_result`, `/showprocesslist/{graph_name}`, and `/showprocesslistall`]* now support internal redirect. You can send the request to any RESTPP node in a cluster and receive a complete response with results from all nodes. An optional parameter disallow_redirect lets you revert to the old behavior and only query the local node.
