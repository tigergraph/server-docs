...
WARNING: The Docker image is preconfigured with a default username and password and so is not a secure environment until xref:_secure_tigergraph[you change the password]. It is not covered by our Support Policy. https://www.tigergraph.com/support-policy/
...
video::kKj5-EGQGa4[youtube,width=640,height=320]
...
+
Please see our xref:installation:hw-and-sw-requirements.adoc#_hardware_recommendations[hardware recommendations].
 ...
 ** Recommended: 8 cores and 24GB memory, or about 80% of what your computer allows.
 ** Minimum: 4 cores and 8 GB memory
...
sudo docker run -d \ <1>
    ...
    -t tigergraph/tigergraph:latest <7>
...
Make sure there are *no spaces after the backslash (\)* on each line. The backslash must be the last character on the line for proper line continuation.
...
[NOTE]
====
If you intend to have multiple instances of TigerGraph in the same Docker environment, whether they are the same version or not, we advise using unique <3> container names and <6> volume names for each one, to avoid conflicts and data overwrite.
====
...
.  xref:user-access:user-management.adoc#_change_a_users_password[Change the password] of the database user called `tigergraph`:
+
[source,console]
----
...
. For additional TigerGraph security settings, see xref:security:index.adoc[].
...
. Run the `gsql` command as shown below to start the GSQL shell. If you are new to TigerGraph, you can run the link:https://github.com/tigergraph/ecosys/blob/master/demos/guru_scripts/docker/tutorial/4.x/README.md[GSQL V3 Tutorial] now.
+
[source,console]
----
$ gsql
