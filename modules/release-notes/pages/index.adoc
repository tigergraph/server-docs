...
TigerGraph 3.5.3 was released on May 10th, 2022 and is available on https://tgcloud.io[TigerGraph Cloud].
...
* Fixed an issue that could crash the Graph Processing Engine (GPE) if `v.getAttr()` was provided the wrong type.
* Fixed an issue that could cause crashes if multiple vertex types had the same attribute names and different positions in rare cases.
* Fixed an issue that could cause crashes due to a problem generating the log files after a data write failure in rare cases.
