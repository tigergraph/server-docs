...
You  can also check the status of each instance using the verbose flag : `gadmin status -v`  or `gadmin status --verbose`. This will show each machine's status along with additional information like *Process ID* and *Role*. See the updated example below:
