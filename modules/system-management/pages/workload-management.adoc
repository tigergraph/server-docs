...
[NOTE]
====
Use a JWT token with `-H "Authorization: Bearer <JWT-token>"` for RESTPP APIs in workload management to ensure secure authentication. `username:password` is supported only for GSQL APIs, not RESTPP.
====
...
[source.warp, bash]
----
curl -X GET -H "Authorization: Bearer <JWT-token>" \
...
On a distributed cluster, you can specify on which replica you want a query to be run through the xref:API:built-in-endpoints.adoc#_run_an_installed_query_post[Run Query REST endpoint].
...
RESTPP supports three query routing modes:

* *Round Robin* (Mode 0): Routes queries across GPEs in round-robin order.
* *CPU Load Aware* (Mode 1): Routes queries to the least-loaded GPE based on CPU usage.
* *Local GPE* (Mode 2): Routes every query to the GPE running on the same node as the receiving RESTPP instance.
...
When `RESTPP.QueryRouting.Mode` is set to `2` (LocalGPE), REST++ forwards each query directly to the GPE co-located on the same node as the receiving RESTPP instance.

This removes the network hop between RESTPP and a remote GPE, reducing query latency for high QPS workloads. It is most beneficial in multi-node or HA deployments where RESTPP would otherwise forward requests to a remote GPE.

You can target a specific GPE by directing your requests to the RESTPP instance running on that GPE's node.

.Example: Enable Local GPE routing
[source, console]
----
$ gadmin config set RESTPP.QueryRouting.Mode 2
$ gadmin config apply
$ gadmin restart RESTPP -y
----
After restarting RESTPP, all incoming queries handled by that RESTPP instance are forwarded to its local GPE.
