...
** This includes replicas used for long-running queries or with the xref:reference:REST/built-in-endpoints.adoc#_headers[GSQL-REPLICA] header.
...
.If an HA system is operating with a failed node, unless the node is recovered, xref:replace-a-node.adoc[replaced], or the system is reconfigured to xref:remove-failed-node.adoc[exclude that node], the following services are limited or unavailable:
...
Loading jobs for xref:data-connect:load-from-kafka.adoc[Kafka] and xref:data-connect:load-local-files.adoc[Local Files] will automatically restart the loader job process if it unexpectedly exits.
