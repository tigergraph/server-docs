...
[NOTE]
====
During a rolling upgrade, the TigerGraph service may be briefly disrupted but should resume on its own.
====

=== Recovering from an Incomplete Rolling Backup

If your rolling upgrade is interrupted or fails to complete, first determine the upgrade status by running:
...
If you receive no message and the upgrade process appears stalled, treat this as a silent failure.

=== How to Proceed After an Incomplete Rolling Backup

When your rolling upgrade does not finish successfully, you have two options:

. Continue with a Manual Offline Upgrade
+
You can manually complete the upgrade by restarting services sequentially:
+
[source.wrap,bash]
----
...
----
+
Then monitor the upgrade progress by running:
+
[source.wrap,bash]
----
...
----
+
Wait until all services are online; this means the upgrade is complete.
...
* Be aware that both manual finishing and reverting may cause temporary service downtime.
====
