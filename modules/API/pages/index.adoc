...
If a larger timeout value is entered, the timeout length will fall back to the value specified in the `RESTPP.Factory.DefaultQueryTimeoutSec` parameter (See xref:reference:configuration-parameters.adoc[]).
