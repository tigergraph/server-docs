...
Use GBAR to xref:tigergraph-server:backup-and-restore:gbar-legacy.adoc[create a backup with gbar] of the primary cluster.
See xref:tigergraph-server:backup-and-restore:gbar-legacy.adoc[Backup and Restore with gbar] on how to create a backup.
...
Copy the backup files from every node to every node on the new cluster.
xref:tigergraph-server:backup-and-restore:gbar-legacy.adoc#_restore_from_a_backup_archive[Restore from a backup archive] of the primary cluster on the DR cluster.

See xref:tigergraph-server:backup-and-restore:gbar-legacy.adoc[Backup and Restore with gbar] on how to restore a backup.
...
[NOTE]
====
In previous version users had to set `System.CrossRegionReplication.Enabled` to `true`, but this is no longer permitted.
