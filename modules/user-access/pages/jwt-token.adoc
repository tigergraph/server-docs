...
[NOTE]
====
The JWT token issued by a third party is limited to authentication on the RESTPP side exclusively. To enable authentication for both GSQL and RESTPP services, it is recommended to generate the token using the GSQL API service.
====
