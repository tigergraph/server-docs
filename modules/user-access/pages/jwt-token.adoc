...
Users may also specify the JWT issuer and audience. The default of these claims are empty:
...
If the user configures this claim and the issuer does not match with a value in the
`aud` string or array or strings, when this claim is present, then the JWT will be
fail.

.Ex: Security.JWT.Audience
[console]
----
$ gadmin config set Security.JWT.Audience "[<audience-name1, audience-name2>]"
----
