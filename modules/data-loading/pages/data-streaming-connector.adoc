...
=== Specify connector configurations
Before creating the connector, you must provide certain connector configurations.
...
The configurations in this subsection specify basic characteristics of the connector such as connector class, the addresses of the source and target clusters, as well as the converters used to covert Kafka Connect format and the serialized form that is written to Kafka.
...
This class of connectors connect a source Kafka cluster to a target Kafka cluster.
...
|Alias of target Kafka cluster in string format
...
To supply the configuration for the corresponding component, replace `<prefix>` with `admin`, `producer`, or `consumer`.
