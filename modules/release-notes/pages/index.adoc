...
TigerGraph 3.5.3 was released on June 1st, 2022 and is available on https://tgcloud.io[TigerGraph Cloud].

Issues fixed:

* Fixed security issues that could have allowed users to access local files on the TigerGraph server.
...
* Fixed an issue that could crash GPE if v.getAttr() was provided the wrong type.
* Fixed an issue that could cause crashes in rare cases where multiple vertex types had the same attribute names and different positions.
* Fixed an issue that could cause crashes in rare cases due to a problem generating the log files after a data write failure.
