...
[#_fixed_issues]
== Fixed issues
* Fixed an issue that could cause failure during cluster shrinking due to communication issues with the ETCD service.
* Resolved several vulnerabilities caused by various dependencies.
* Fixed an issue that could cause system to become unresponsive due to the executor service dysfunction.
* Fixed an issue where exceedingly long queries (over 1000 lines) take a long time to be created and installed.
* Fixed an issue that caused GBAR backup to be unable to reach AWS S3 on private network.
...
* Fixed an issue that led to GSE dysfunction during leader switch.
