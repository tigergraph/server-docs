...
* [3.10.4] xref:3.10@tigergraph-server:reference:configuration-parameters.adoc#_zk[New ZK.SnapCount configuration] enables users to fine-tune the number of committed ZooKeeper transactions that trigger a snapshot, enhancing memory management and stability under high transaction loads.
...
* Fixed an issue where vertex attributes loaded with invalid values when a loading job inserted vertices using reducer operators such as `and()`, `ignore_if_exists()`, `max()`, or `min()` (CORE-4866).
* Fixed data rejection during loading jobs by enhancing timestamp conversion to support a wider range of dates, including those prior to 1970-01-01 (GLE-10579).
* Fixed an issue where retrieving the query list in Write Queries would get stuck when creating a large query (GLE-10461).
* Fixed an issue where users could not locate a vertex in the Graph Studio result when the executed query only printed edges (APPS-3991).
* Fixed an issue where an incorrect 'query not running' error message appeared when aborting a query in the Admin Portal on multi-node systems (APPS-3962).
* Fixed an issue where the delete query button was incorrectly enabled for queries that were only saved but not yet installed, causing an error when attempting to delete them (APPS-3917).
* Fixed an issue where query changes were lost or reverted when clicking the save button immediately after editing or fixing syntax errors (APPS-3916).
* Fixed issue where codecheck failed to prevent the installation of a query with syntax errors in Write Queries, by catching unexpected exceptions and reporting errors, rather than skipping the query (GLE-10467).
* Fixed issue with exporting only vertexset attributes to CSV when using the `print <vertexset> TO_CSV <file>;` statement (GLE-10521).

==== Improvements

* Upgraded Nginx to version 1.26 (TP-8129).
* Removed authentication requirements for replaying requests in Disaster Recovery (DR) setups, streamlining Cross-Region Replication (CRR) operations (GLE-10601).
