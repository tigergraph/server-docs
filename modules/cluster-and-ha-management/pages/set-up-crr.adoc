...
Use `gadmin backup` to xref:tigergraph-server:backup-and-restore:backup-cluster.adoc[]. If you are restoring a backup from another cluster refer to xref:tigergraph-server:backup-and-restore:cross-cluster-backup.adoc[].
