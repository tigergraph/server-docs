...
The value of `"parameters"` is an object containing a name-value pair for each parameter.
...
| Vertex, type specified in query defintion
| _vertex_id_
| `"source": "v34261"`
...
If the parameter type is a set or list, then use JSON array syntax `[item1, item2, ..., itemN]` for the parameter value, where each item follows the rules in the table.
