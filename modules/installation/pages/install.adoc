...
Below is a description of the fields in the config file:

* `"BasicConfig"`
 ** `"TigerGraph"` : Information about the TigerGraph Linux user, who is the Linux user that will be created by the installer to manage the TigerGraph platform. Commands such as `gadmin` are only available to the TigerGraph Linux user.
...
If you use key-based authentication, the sudo user you are providing must have password-less root privileges on all nodes in the cluster.
  ...
  *** `"ReplicationFactor"`: xref:ha:ha-cluster.adoc[Replication factor] of the cluster.
   **** If you would like to enable the xref:ha:ha-cluster.adoc[High Availability (HA) feature], please make sure you have at least 3 nodes in the cluster and set the replication factor to be greater than 1.
For example, if your cluster has 6 nodes, you could set the replication factor to be 2 or 3.
If you set the replication factor to be 2, then the partitioning factor will be  6 / 2 = 3.
Therefore, 3 nodes will be used for one copy of the data, and the other 3 nodes will be used as a replica copy of the data.
