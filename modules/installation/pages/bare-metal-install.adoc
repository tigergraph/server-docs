...
* *RegionAware*: For more information on the significance and requirements for setting the `RegionAware` parameter, see xref:cluster-and-ha-management:region-aware.adoc[].
...
** Users need to provide region information to enable xref:cluster-and-ha-management:region-aware.adoc[region-aware]:
+
...
[NOTE]
====
Either the root user or a Linux user with passwordless sudo permissions can run the installation.
====
