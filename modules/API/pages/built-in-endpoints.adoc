...
The maximum size of data you can upload via this endpoint is controlled by the link:intro.adoc#_request_body_size[`Nginx.ClientMaxBodySize`] configuration parameter (default is 200 MB).
...
|`new_vertex_only` |No |Boolean value that indicates whether or not to
update existing vertices. If the value is true, it will only insert new
vertices and not update existing ones.

|`vertex_must_exist` |No |Boolean value that indicates whether or not to
insert new edges when the `FROM` or `TO` vertices don’t exist. If the
value is true, the request will only insert edge if both the `FROM` and
`TO` vertices of the edge already exist. If the value is false, the
request will always insert new edges, and create the necessary vertices
with default values for their attributes.
|===

The response is the number of vertices and edges that were accepted. The API uses JSON format to describe the vertices and edges to be upserted. The JSON code can be stored in a text file or specified directly in a command line. There is a maximum size for a `POST` data payload (see the link:intro.adoc#_size_and_time_limits[*Size Limits*] section). The JSON format for describing a vertex set or edge set is summarized below.
...
Follow the instructions in the Introduction section to link:intro.adoc#_formatting_data_in_json[format advanced data types]. For example, the following payload is used to upsert two `User` vertices with an attribute `coordinates` of type `LIST` and an attribute `measurements` of type `MAP`:
...
To insert vertices or edges, use the link:built-in-endpoints.adoc#_upsert_data_to_graph[Upsert data to graph] endpoint.
...
When using a `GET` request to run an installed query, the query parameters are link:intro.adoc#_query_string_parameter_passing[passed in through the query string of the URL].
...
When running an interpreted query through this endpoint, the query parameters should be link:intro.adoc#_query_string_parameters[passed in through the URL query string].
...
| `expirationTime`
| The timestamp for when the given query times out. The default timeout limit is 16 seconds and can be set using the link:intro.adoc#_gsql_query_timeout[`GSQL-TIMEOUT`] header.
...
Other versions of pathfinding algorithms are available in the xref:graph-ml:intro.adoc[GSQL Graph Algorithm Library].
