...
=== Developer tools
* Introduced xref:insights:intro:index.adoc[TigerGraph Insights (Preview)], a no-code visual graph analyzer that makes building data analytics dashboards intuitive.
...
* Expanded xref:gui:admin-portal:dashboard.adoc[metrics] available at a glance through Admin Portal.
* Enhanced ease of use by enabling drag-and-drop xref:gui:graphstudio:map-data-to-graph.adoc[data mapping in GraphStudio] and in-browser JSON file upload.
* Added new xref:gui:graphstudio:explore-graph/actions.adoc[] page in the GraphStudio GUI for easy, secure commands to clear and rebuild graph data.
...
Every time a user performs an action that requires any privilege, a detailed log of the action and the user is generated.
...
* xref:gsql-ref:ddl-and-loading:functions/token/index.adoc[Token functions for attribute expressions] can now also be used in expressions in the `WHERE` clause.
