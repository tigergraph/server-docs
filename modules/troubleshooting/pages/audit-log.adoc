...
.You can modify the lifetime of file with the following command:
[source, console]
----
gadmin config set Gadmin.BasicConfig.LogConfig.LogFileMaxDurationDay: <lifetime of file>
----
...
.You can modify the the rotation file number with the following command:
[source, console]
----
...
Besides from the basic information such as `timestamp`, `OS`, `username`, they capture the `gadmin` commands executed on the node and the outcomes:
