...
¦Using xref:{page-component-version}@gsql-ref:querying:accumulators.adoc#_functions[`.flip()`] on `BitwiseAndAccum` or `BitwiseOrAccum` inside a `PRINT` block may produce different results in UDF mode versus Single/Distributed mode.
¦4.1.3
