...
| Provide the URL for the public keys used to verify JWT signatures. The URL points to a xref:4.1@tigergraph-server:user-access:jwt-token.adoc#_setup_jwt_token_gadmin_configuration[JWKS (JSON Web Key Set)] endpoint.
| `https://jwt.example.com/.well-known/jwks.json`

| Security.JWT.RSA.PublicKey
| Configure a RSA public key for xref:4.1@tigergraph-server:user-access:jwt-token.adoc#_setup_jwt_token_gadmin_configuration[OIDC JWT Authentication].
| `gadmin config set Security.JWT.RSA.PublicKey <public-key content or URL or @filepath>`

| Security.JWT.HMAC.Secret
| Configure a HMAC Secret for xref:4.1@tigergraph-server:user-access:jwt-token.adoc#_setup_jwt_token_gadmin_configuration[OIDC JWT Authentication].
