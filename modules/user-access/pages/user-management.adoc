...
Use the `SHOW PROXY USER` command to view proxy user metadata.

=== Syntax

[source,gsql]
----
SHOW PROXY USER <proxy_user_name> (, <proxy_user_name>)*
----
...
If the current user has `READ_USER`, information for all users (local and proxy) is displayed.
