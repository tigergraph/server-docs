...
//:page-aliases: import-export:database-import-export.adoc
...
To see how to Export/Import selected individual graphs in a database refer to xref:backup-and-restore:single-graph-import-export.adoc[].

[NOTE]
====
Beginning with version 4.1, a full export package now includes access policies and template queries. LDAP proxy groups are not included.
====

[IMPORTANT]
====
Import/export is a complement to xref:backup-and-restore:index.adoc[], but not a substitute. See the xref:#_known_issues[] for additional considerations.
====
...
[WARNING]
====
* User-defined loading jobs containing xref:{page-component-version}@gsql-ref:ddl-and-loading:creating-a-loading-job.adoc#_delete_statement[`DELETE` statements] are not exported correctly.
...
[NOTE]
====
Sensitive information included in exported graph files, such as passwords, access keys, secret keys, and tokens, is encrypted before being written to the export package.

As a result, exported files may contain encrypted credential values that are required for import operations and do not expose the original plain-text credentials.
====
...
Please refer to the section on xref:{page-component-version}@gsql-ref:querying:func/query-user-defined-functions.adoc#udf-security[UDF Security] to ensure that UDFs comply with the security specifications. This will help you import the solution successfully.
====
...
If you're exporting from a single-node cluster, the `ExportedGraph.zip` can be directly imported, so you don't need the source and target clusters to have the same number of servers. 
If you're exporting from a smaller cluster to a larger one, you must transfer the dataset files along with the zip file to the appropriate nodes in the target cluster. 
