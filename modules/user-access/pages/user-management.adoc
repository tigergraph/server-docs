...
*Usernames* can contain any Unicode characters
or any valid email *except* the following:

. `\r`, `\n`, `\0`, `\b`, `\t`,

. Backquotes (```) can be included in usernames

`jack:@email.com`

. Cannot start with a dot `.` or have multiple dots in a sequence.
...
----
+
.Example 3: Create user with special characters
+
[source,console]
----
User Name : `:"/.,@#$%^*()_+=-`
