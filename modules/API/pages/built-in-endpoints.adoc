...
In 4.2.1, this endpoint provides a complete view of all running queries on all nodes for the specified graph.
...
[source.wrap,bash]
----
curl -X GET "http://localhost:14240/restpp/showprocesslist/poc_graph"
----
--
Response::
+
...
In 4.2.1, this endpoint provides a complete view of all running queries on all nodes in the cluster.
...
[.wrap,console]
----
curl -X GET "http://localhost:14240/restpp/showprocesslistall"
----
Response::
+
----
...
In 4.2.1, you can send the request to any live RESTPP node, and it will return the complete status from across the cluster.
...
In 4.2.1, you only need to send your request to any live RESTPP node. The endpoint returns query results regardless of which RESTPP node handled the workflow. 
