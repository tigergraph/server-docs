...
* [4.1.3] Added support for the `Security.JWT.JWKS.URL` xref:4.1@tigergraph-server:user-access:jwt-token.adoc#_setup_jwt_token_gadmin_configuration[configuration parameter], allowing JWT certificates to be fetched using the Key ID (kid) from a JWKS (JSON Web Key Set) URL.
