...
** This includes replicas used for long-running queries or with the xref:API:built-in-endpoints.adoc#_headers[GSQL-REPLICA] header.
...
.If an HA system is operating with a failed node, unless the node is recovered, xref:how_to-replace-a-node-in-a-cluster.adoc[replaced], or the system is reconfigured to xref:remove-failed-node.adoc[exclude that node], the following services are limited or unavailable:
...
NOTE: As a workaround, if the failed node cannot be recovered (e.g. hardware issue), full operation can be restored temporarily by the xref:remove-failed-node.adoc[removal of the failing nodes].
...
In this case, as with the other consideration cases above, the failed node needs to be xref:remove-failed-node.adoc[removed] from the cluster via:
...
== xref:ha-for-gsql-server.adoc[High Availability Support for GSQL Server]

Learn how TigerGraph incorporates built-in HA for all the internal critical components.

== xref:ha-for-application-server.adoc[High Availability Support for Application Server]
...
== xref:cluster-commands.adoc[Cluster Commands]
...
== xref:remove-failed-node.adoc[Removal of Failed Nodes]

Here you find detailed Instructions for the removal of a failed node.

== File and Kafka loaders HA with Auto-Restart

Loading jobs for xref:data-loading:load-from-kafka-mm.adoc[Kafka] and xref:data-loading:load-local-files.adoc[Local Files] will automatically restart the loader job process if it unexpectedly exits.
