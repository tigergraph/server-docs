...
== what will happen if password has expired
...
when the password will be expired?

For example, if the password is created now, and we configure it with command `gadmin config set Security.UserPasswordPolicy.ExpirationDay 2` . The passsword will be expired in 2 days.
