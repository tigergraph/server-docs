...
** Added xref:3.9@tigergraph-server:data-loading:load-from-cloud.adoc#_continuous_loading_from_cloud_storage[auto-detection of new files] when in Stream mode.
...
* [3.9.3] xref:3.9@tigergraph-server:user-access:access-control-model.adoc#_data_crud_privileges[Edge operations] now require privileges on endpoint vertex IDs. To create, read, update, or delete an edge, the corresponding privilege (`CREATE_DATA`, `READ_DATA`, `UPDATE_DATA`, `DELETE_DATA`) must also be present on the primary IDs of the source and target vertices, respectively.
...
* Fixed a permissions inconsistency that required an Admin Portal user to have the `EXECUTE_LOADINGJOB` privilege in order to create a secret (APPS-2568). 
...
* v3.9.3
** Performing edge operations now requires both edge-level and vertex-level privileges. Specifically, to create, read, update, or delete an edge, the corresponding privilege (`CREATE_DATA`, `READ_DATA`, `UPDATE_DATA`, `DELETE_DATA`) must also be granted on the primary IDs of the edge’s source and target vertices, respectively.
