...
After creating the proxy group, you can xref:access/user-management.adoc[grant roles] to the group to give the proxy group privileges.
...
[NOTE]
====
Proxy groups cannot be xref:backup/database-import-export.adoc[exported].However, performing a xref:backup/index.adoc[backup and restore] will preserve all proxy groups present at the time of backup.
====
