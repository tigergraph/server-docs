...
[NOTE]
====
Proxy groups are not included in a  xref:backup-and-restore:database-import-export.adoc[Database Export].
They need to be recreated following an Import.
However, xref:backup-and-restore:index.adoc[Backup and Restore] do preserve all proxy groups present at the time of backup.
====
