...
WARNING: This data loader is deprecated and no longer supported.
...
----
{
    "file.uris": "s3a://my-bucket/data.csv"
}
----
...
----
{
    "file.uris": "s3a://my-bucket1/data1.csv,s3a://my-bucket1/data2.csv,s3a://my-bucket2/data3.csv"
}
----
...
----
DEFINE FILENAME f1 = "$s1:~/files.config";
DEFINE FILENAME f1 = "$s1:{\"file.uris\":\"s3a://my-bucket/data.csv\"}";
----
...
For example, the URI``+s3a://my-bucket/${yyyy}+`` is converted to ``s3a://my-bucket/2023`` when running the loader.
You can use as many as you like in the URIs, for instance:``+s3a://my-bucket/${yyyy}/${MM}/${DD}/${HH}-${mm}+``
====
...
Here is an example of loading data through the S3 Loader:
