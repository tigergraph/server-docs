...
So at this point the DR cluster has at least two sets of internal topics, one initialized during installation and one created by CRR synchronization. It is through this configuration `System.CrossRegionReplication.TopicPrefix` that the DR cluster decides which set of internal topics to use.
If the situation becomes more complicated, suppose this DR cluster is switched to a PR cluster, this configuration must remain unchanged. When we need to setup a new DR cluster for this cluster, the CRR component will add the "primary." prefix again when it creates a replica of the topics, so this configuration needs to be set to "primary.primary." for the new DR cluster.
====
