...
[source,bash]
----
curl -X POST -H "content-type: text/plain" -H 'Workload-Queue: <your_workload-query_name>' 'localhost:14240/gsql/v1/queries/interpret' -d 'INTERPRET QUERY (SET<STRING> p1) FOR GRAPH financialGraph syntax v1 {  print p1; }'
...
[source,bash]
----
curl -X POST -H "content-type: text/plain" "http://localhost:14240/gsql/v1/queries/interpret?gsql=true" -d 'INTERPRET QUERY -QUEUE <your_worload_queue_name> (SET<STRING> p1) FOR GRAPH financialGraph syntax v1 {  print p1; }' 
----
