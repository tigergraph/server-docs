...
If your vertex has composite keys, separate the attributes that make up the composite key with comma (`,`) in the same order as they are defined in the schema.

For example, suppose we have the following vertex definition:

[.wrap,gsql]
----
CREATE VERTEX compositePerson(id UINT, name STRING, age UINT, primary key (name, id))
----

The following requests upserts two vertices with the defined composite key, as well as an edge of the type `compositeRoles` between `Bob, 123` and `Harry Potter, 1990, 1337`:
