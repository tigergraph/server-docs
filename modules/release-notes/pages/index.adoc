...
NOTE: TigerGraph Server 3.5.0 is available on xref:cloud:release-notes:release-notes.adoc[TigerGraph Cloud].

== Patch releases

=== 3.5.1

TigerGraph 3.5.1 was released on March 30th, 2022.

Issues fixed:

* Fixed an issue that caused GraphStudio to issue too many expensive requests if the user stays on the Load Data page.
* Fixed an issue that in rare cases prevented Graph Processing Engine (GPE) process from restarting after being terminated.
...
* Fixed an issue that in rare cases prevented TigerGraph cluster from recovering from a Kafka failure.
* Resolved SQL injection risks in GraphStudio.
* Added a warning when a query written in syntax V2 uses the xref:gsql-ref:querying:select-statement/index.adoc#_sample_clause[`SAMPLE` clause] of a `SELECT` statement.
...
* Added support for generic vertex in xref:gsql-ref:querying:data-modification-statements.adoc#_insert_into_statement[`INSERT INTO <edge_type>` statements].
...
* (New in 3.5.1) Added the ability to
...
=== Fixed in 3.5.0
* Fixed an issue where the installer does not check `AllowerUser` on all nodes for installation on OEL8.
