...
¦Graph-level secrets are no longer supported starting with 4.1.0. During an upgrade from a version earlier than 4.1.0, existing graph-level secrets are migrated to global-level secrets. Tokens associated with the migrated graph-level secrets are removed during the migration.
¦4.1.0+
a¦N/A
¦N/A
