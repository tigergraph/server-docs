...
* [Add link] GSQL Data Streaming Improvements - Improvements to GSQL data streaming that reduce the CPU usage, improve performance, optimizes Disk usage, and increases stability and cohesion.
...
* [Add link] Global Schema Restrict to Global Scope - Users must have the appropriate scope to interact with global schema change jobs (create, delete, run).

* [Add link] Kafka Security via SSL - Kafka brokers can be secured by SSL including the connections from Kafka clients to Kafka brokers.
...
* [Add link] Support for Standard OIDC Authorization Code Flow - Support for OIDC SSO with code flow for general purpose adds more security.
