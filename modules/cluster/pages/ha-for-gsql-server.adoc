...
When a cluster is xref:get-started:install/bare-metal-install.adoc[initially installed] or xref:cluster:repartition-a-cluster.adoc[resized], the installer will automatically select an appropriate number of service components.
