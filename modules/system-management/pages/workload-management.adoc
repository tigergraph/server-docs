...
For example, in a TigerGraph cluster with `4` nodes (there will be `4` `GPE` processes), the total number of queries allowed for a `WorkloadQueue` is `4*maxConcurrentQueries`.
Similarly, the total number of queries can be put into the corresponding delay queue is `4*maxDelayQueueSize`.
TigerGraph internally would try to evenly distribute queries among the nodes, hence, the `WorkloadQueue` from each `GPE` would be filled at a similar pace.
...
[source.wrap,gsql]
----
GET WORKLOAD QUEUE TO "/path/to/queue.json"
----
...
This is ignored once the xref:#_workload_queues[workload queue] feature is enabled.
