...
For a regular expression match users need to add a prefix `regex:` before creating the match rule.
...
.Ex: `\b` is the word boundary used in regular expressions. So, if a user `nameid` contains `abc` they will match this group.
[source, console]
----
