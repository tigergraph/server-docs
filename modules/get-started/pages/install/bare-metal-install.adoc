...
* xref:cluster:ha-cluster.adoc[]
* xref:cluster:crr-index.adoc[]

NOTE: If you are performing an upgrade of an existing TigerGraph installation, please see xref:install/upgrade.adoc[] instead.

== Prerequisites

* Your machines meet the minimum xref:install/hw-and-sw-requirements.adoc[_Hardware and Software Requirements_].
...
* *TigerGraph Username and Password*: A TigerGraph database has a built-in superuser, whose default username and password are `tigergraph`.
You can either customize the username and password now or after installation.
...
* *SSHPort*: By default, the SSH port is `22`.
If this port is open in all the provisioned machines you can leave it as it is.
Otherwise, set this parameter to the actual SSH port in your environment.
...
* *RegionAware*: For more information on the significance and requirements for setting the `RegionAware` parameter, see xref:cluster:region-aware.adoc[].
...
** Users need to provide region information to enable xref:cluster:region-aware.adoc[region-aware]:
+
...
+
image:install-using-hostname.png[]
