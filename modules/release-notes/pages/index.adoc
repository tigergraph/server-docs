...
=== Manageability
* Added support to backup and restore for xref:kubernetes:index.adoc[TigerGraph deployments on Kubernetes].
...
* xref:3.4@tigergraph-server:release-notes:index.adoc[Release notes - TigerGraph 3.4]
* xref:3.3@tigergraph-server:release-notes:index.adoc[Release notes - TigerGraph 3.3]
* xref:3.2@tigergraph-server:release-notes:index.adoc[Release notes - TigerGraph 3.2]

== Compatibility with TigerGraph 3.4
* GSQL Syntax version change.
Beginning with TigerGraph 3.5, the default GSQL syntax version is V2.
Syntax V1 is still supported by TigerGraph, but queries written in V1 syntax will need the version specifier at the query level or a session parameter specifying the syntax version to compile.
