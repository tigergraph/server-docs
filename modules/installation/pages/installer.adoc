...
TigerGraph suggests to install TigerGraph with the following steps:

* Install all needed tools with `./install_tools.sh`
* Edit the `install_conf.json` according to your needs and install TigerGraph in non-interactive mode with `./install.sh -n -N -F`
