...
|3.6.0
|When vertices are automatically created during edge loading, the primary key of the of vertices created are not written to vertex attributes, even if the vertex definition has `primary_key_as_attribute` set to true.
|Unresolved


|3.6.0
|Running a query with `edgeAttribute()` or `neighborAttribute()` function when accumulator containing universal vertex types is referenced in the same statement cause GPE dysfunction.
|Unresolved
