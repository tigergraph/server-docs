...
[NOTE]
====
Rolling Upgrade is only available for HA clusters (High Availability) and is supported for maintenance releases (e.g., 4.2.0 to 4.2.1, 4.3.1 to 4.3.3). 
Upgrading from 4.2.0 to 4.3.0 or any other major version is not supported.
====

During the rolling upgrade, certain operations are not supported, including installing queries, running schema changes, dropping all data, using `gsql --reset`, creating graphs, backup/restore, clearing graph stores, exporting/importing graphs, and installing functions. Additionally, occasional drops in queries per second (QPS) may occur. 
The following services will not be upgraded during the rolling upgrade process: Zookeeper (ZK), Kafka, and ETCD.
...
After the binaries and configuration files are installed, user will be prompted with the following message:
...
[source.wrap,bash]
----
$ gadmin config set System.AppRoot /home/tigergraph/tigergraph/app/4.2.0
$ gadmin config apply -y
$ gadmin restart all -y
----
...
*Manual Upgrade*: Execute the following commands to restart services in sequence:
