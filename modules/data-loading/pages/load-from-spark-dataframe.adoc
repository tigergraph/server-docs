...
== Overview
TigerGraph Spark loading is leveraging xref:tigergraph-server:API:built-in-endpoints.adoc#_run_a_loading_job[loading job built-in endpoint].
...
.The processed data will be sent to TigerGraph like CSV files to run the loading job:
...
[NOTE]
====
Authenticate with the username/password or secret to support token refresh, otherwise, make sure the lifetime of the token is long enough for the loading job.
====
