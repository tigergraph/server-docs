...
This page provides detailed procedures for migrating ACL-based query privileges in 3.x to object-based privileges in 4.1+.
It also outlines the specific privileges required to successfully carry out the migration process.

To see Role-based Access Control(RBAC) Object-Based privileges, see xref:user-access:rbac-row-policy/rbac-row-policy.adoc#_object_based_privileges[].

== Migrate ACL Privileges
Starting with TigerGraph 4.1, Access Control Lists (ACLs) are no longer available.

ACL privileges on queries are not migrated automatically during upgrade. Before
upgrading from version 3.x, you must manually revoke all ACL privileges on
queries. If the ACL catalog still contains query contents at upgrade time, the
upgrade is blocked.

To preserve the same access levels after upgrade, manually grant the following
privileges to the users/roles that previously held them under ACLs:

* `OWNERSHIP`: to the owner, on the query they owned
* `READ`: to readers, on the same query
* `EXECUTE`: to executors, on the same query

[NOTE]
====
If ACL privileges are still present during upgrade, the system displays this message:

----
The ACLPrivilegeCatalog.json contains the hidden query contents.
Please revoke all ACL READ privileges on queries before upgrading to 4.x.
----
