...
During an expansion, you can also change the xref:ha:ha-cluster.adoc[replication factor] of your cluster.
...
The overall amount of space required for expansion on each node is `(1 + Math.ceil(oldPartition/newPartition) ) * dataRootSize]`.
...
When the expansion completes, you'll see a message confirming the completion of the cluster change.
The message also includes the location of the temporary files created during the expansion.

Verify data integrity by comparing vertex/edge counts or query results.
After confirming a successful expansion, delete the temporary files to free up disk space.
