...
This endpoint retrieves multiple vertices and edges in a single request, given their types and IDs.
...
{
  "vertices": [                                          (1)
    { "type": "<vertex_type>", "id": "<vertex_id>" }
  ],
  "edges": [                                              (2)
    {
      "type": "<edge_type>",
      "source": { "type": "<source_vertex_type>", "id": "<source_vertex_id>" },
      "target": { "type": "<target_vertex_type>", "id": "<target_vertex_id>" },
      "discriminator": { "<attribute_name>": "<value>" }  (3)
    }
  ]
}
----
<1> `vertices`: list of vertices to retrieve, each identified by `type` and `id`.
<2> `edges`: list of edges to retrieve, each identified by edge `type`, `source` (vertex type and ID), and `target` (vertex type and ID).
<3> `discriminator`: required only if the edge type has a discriminator.
