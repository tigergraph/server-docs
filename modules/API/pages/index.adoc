...
You can request a gzip-compressed response for a query by including the `Accept-Encoding: gzip` header in your request. This is supported for query responses larger than 1000 bytes.

Use this option when network bandwidth is low. gzip compression itself takes some processing time.

[source,bash]
----
curl -X GET -H "Accept-Encoding: gzip" -o /path/to/zip.gz "http://localhost:14240/restpp/query/{graph_name}/{query_name}"
----

You can also request a gzip-compressed response when running a query from the GSQL shell , see xref:gsql-ref:querying:run-manage-queries.adoc#_gzip_compression[`-gzip` option] of the `RUN QUERY` command.

==== Zstd compression

You can request a zstd-compressed response for an installed query by including the `Accept-Encoding: zstd` header in your request.

[source,bash]
----
curl -X GET -H "Accept-Encoding: zstd" -o /path/to/out.zst "http://localhost:14240/restpp/query/{graph_name}/{query_name}"
----
...
You can also request a zstd-compressed response when running an installed query from the GSQL shell. See xref:gsql-ref:querying:run-manage-queries.adoc#_zstd_compression[`-zstd` option] of the `RUN QUERY` command.

NOTE: If you submit or run a query in async mode, do not use gzip or zstd compression with the request. Use gzip or zstd compression with the `query_result` API to fetch the actual query result.
 ...
 ** TigerGraph APIs use headers to specify xref:API:index.adoc#_size_and_time_limits[size and time limits], as well as to provide xref:user-access:jwt-token.adoc#_usage_of_gsql_jwt_token[authentication tokens].
* `-s`
