...
* To create an edge, you must also have `CREATE_DATA` privilege on the primary ID attributes of its source and target vertex types.
...
To read an edge or its attributes, you must also have `READ_DATA` privilege on the primary ID attributes of its `FROM` and `TO` vertex types.
...
When creating an edge, you must have `CREATE_DATA` privilege on the primary ID attributes of both the source and target vertex types, in addition to `CREATE_DATA` on the edge type itself.

Creating an edge references specific vertices by their IDs, and the system requires permission to validate those IDs. Without this, the system returns a permission error, even if you have `CREATE_DATA` on the edge type itself.
