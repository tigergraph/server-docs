...
このエンドポイントによってアップロードできるデータの最大サイズは、xref:API:intro.adoc#_応答サイズ[ `Nginx.ClientMaxBodySize` ]設定パラメーター(デフォルトは200MB)によって制御されます。
...
「はじめに」セクションの指示に従って、xref:API:intro.adoc#_jsonでのデータフォーマット[高度なデータタイプのフォーマット]を行ないます。
...
頂点またはエッジを挿入するには、xref:API:built-in-endpoints.adoc[データのグラフへのアップサート]エンドポイントを使用します
...
`GET` リクエストを使用してインストールされたクエリーを実行する場合、クエリーパラメーターはxref:API:intro.adoc#_クエリ文字列パラメーター[URLのクエリー文字列を通じて渡されます]。
...
`POST` リクエストを使用してインストールされたクエリーを実行する際、クエリーパラメーターはリクエスト本文を通して、xref:API:intro.adoc#_jsonでのデータフォーマット[JSON形式でエンコード]した形で渡されます。JSONペイロードのフォーマットルールは、xref:3.2@gsql-ref:querying:query-operations.adoc#_parameter_json_object[JSONを使用した `RUNQUERY` コマンドでのパラメーターの受け渡し]の場合と同様です。
...
このエンドポイントを通じてインタープリテッドクエリーを実行する場合、クエリーパラメーターはxref:intro.adoc#_クエリ文字列パラメーター[URLクエリー文字列を通じての受け渡し]が行なわれる必要があります。
...
[#_check_query_status_detached_mode]
...
| `expirationTime`
| 指定されたクエリがタイムアウトしたときのタイムスタンプ。デフォルトのタイムアウト制限は16秒で、xref:intro.adoc#_gsqlクエリのタイムアウト[`GSQL-TIMEOUT`]ヘッダーを使って設定可能です。
...
パス検索の他のバージョンは、xref:graph-ml:intro:index.adoc[GSQLグラフアルゴリズムライブラリー]でご覧ください。
