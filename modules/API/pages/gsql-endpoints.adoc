...
|auth_methods |No |Allowed SSO authentication methods (`KERBEROS`, `OIDC`, `SAML`). If specified, the proxy user can log in only using these methods. Login fails if a different SSO method is used.
...
{
  "error": false,
  "message": "Successfully created proxy user test_user"
}
----
--
====
