...
* Some UDFs may no longer be accepted due to xref:manage:security/index.adoc#_udf_file_scanning[increased security screening].
...
. First perform a backup xref:manage:backup/index.adoc[using GBAR (Graph Backup and Restore)] on your existing installation.
...
You can do this by calling the xref:reference:REST/built-in-endpoints.adoc#_rebuild_graph_engine[/rebuildnow] endpoint and waiting until there are no more PullDelta messages being printed in the logs.
...
Call the xref:reference:REST/built-in-endpoints.adoc#_rebuild_graph_engine[/rebuildnow] endpoint to force the data store to consume all pending updates and then wait until there are no more PullDelta messages being printed in the logs.
...
. xref:manage:backup/gbar-legacy.adoc[Back up] your TigerGraph instance using GBAR.
. Start a new instance from the latest cloud marketplace listing.
. Use the backup files you generated earlier to xref:manage:backup/gbar-legacy.adoc[restore] the new instance.
...
For more reference on how to prepare for an upgrade please refer back to the section: xref:install/upgrade.adoc#_before_you_begin[Before You Begin].
...
For more on file policy see xref:manage:security/file-output-policy.adoc[] and/or xref:manage:security/gsql-file-input-policy.adoc[].
