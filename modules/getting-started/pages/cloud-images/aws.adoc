...
This tutorial will show you how to start the TigerGraph database from an image on AWS.
...
The TigerGraph Database cloud image comes with *a perpetual license* that will *only work on the AWS instance it's installed on*. Please run the following command to see it:
