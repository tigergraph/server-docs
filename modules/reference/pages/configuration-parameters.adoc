...
This page provides a comprehensive list of the parameters available for configuration with `gadmin config`.
...
|Controller.Connect.PruneIntervalMin |The waittime between each prune
job which removes data already loaded by GSQL in kafka |`1`
...
|`LD_PRELOAD=$LD_PRELOAD; LD_LIBRARY_PATH=$LD_LIBRARY_PATH; CPUPROFILE=/tmp/tg_cpu_profiler; CPUPROFILESIGNAL=34; MALLOC_CONF=prof:true,prof_active:false; ZMQ_KEEPALIVE=0`
...
|RESTPP.WorkLoadManager.MaxHeavyBuiltinQueries |The maximum concurrent
running “heavy“ built-in queries(kstep_expansion, searchvertex,
