...
[NOTE]
====
Rolling Upgrade is only available for HA clusters (High Availability) and when making a maintenance update (e.g., 4.2.0 to 4.2.1, 4.3.1 to 4.3.3). 
Upgrading from 4.2.0 to 4.3.0 or any other major version is not supported.
====

During a rolling upgrade, several operations are not permitted to ensure a smooth upgrade process. 
These operations include installing queries, running schema changes, dropping all data, using `gsql --reset`, creating graphs, performing backup/restore operations, clearing the graph store, exporting/importing graphs, installing functions, changing TigerGraph configurations, starting or stopping services, setting up CRR (though CRR will continue working if already set up), running `gadmin init` for cluster initialization, upgrading services with `gadmin upgrade`, and performing service resets with `gadmin reset`. 
While the rolling upgrade is in progress, loading jobs and running queries will continue to function; however, queries requiring all replicas to be online may fail, in which case you can retry them in a round-robin manner. 
Please note that occasional slowdowns in Queries Per Second (QPS) may occur during the upgrade. Additionally, services like Zookeeper (ZK), Kafka, and ETCD will not be upgraded during this process.
...
=== Managing Service Disruption During Rolling Upgrade

During a rolling upgrade, temporary service interruptions are expected. If the upgrade process fails or the connection is unexpectedly lost, you can resume the upgrade by running:
...
=== Rolling Upgrade Failure with Error

If the rolling upgrade encounters an error, the error message will guide you on how to proceed. To manually finish the upgrade, follow these steps:

. Restart the services sequentially with this command:
+
[source.wrap,bash]
----
...
. Open another terminal and run the following command to monitor service status:
+
[source.wrap,bash]
----
watch -n 2 gadmin status -v
----

. Wait until all services are online. This indicates the upgrade is complete.

. If you need to revert to the *previous version*, run these commands:
+
[source.wrap,bash]
----
...
Replace `<old version>` with the version you were running before the upgrade (e.g., `4.2.0`).

=== Rolling Upgrade Failure Without Error

In rare cases, the rolling upgrade may fail without displaying any error message. For example, this may happen if the controller node is accidentally restarted during the upgrade. If this occurs, you may not receive an error message or indication of failure.
If the process halts and you do not receive a clear success message or an error after running:

[source.wrap,bash]
----
$ gadmin upgrade --resume
----
It should be considered a silent failure.

In such cases, you can either *manually switch to the new version* or *revert to the previous version*:

*Switch to the New Version*

To manually switch to the new version, run:
...
Replace `<new version>` with the target version number (e.g., `4.2.1`).

*Revert to the Previous Version*

To revert back to the previous version, run:
...
Replace `<old version>` with the previous version number (e.g., `4.2.0`).

[NOTE]
====
These manual steps for finishing or reverting upgrades may cause service downtime.
====
