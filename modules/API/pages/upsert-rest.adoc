...
In 4.2, we’ve introduced support for a standard JSON array format for upserting multiple edges with different discriminators in a single request. 
Previously, the JSON format required non-standard practices (e.g., reusing the same key for multiple values), which has now been replaced with the array format for greater flexibility and ease of use.

Regular edges do not have discriminators and are uniquely defined by their source and target vertex IDs. The format for upserting regular edges remains unchanged.
...
Edges with xref:gsql-ref:ddl-and-loading:defining-a-graph-schema.adoc#_discriminator[discriminators] allow multiple instances of the same edge type between two vertices, each distinguished by a unique discriminator attribute.

=== Standard JSON Array Format for Multi-Edges with Discriminators

To insert multiple edges with discriminators between the same source and target vertices, use the *standard JSON array format*. This format supports inserting multiple edges in a single operation.
...
To upsert multiple edges with discriminators using the JSON array format, use the following structure:
...
This original format for upserting multi-edges with discriminators is still supported for backward compatibility. 
You can continue using this format if preferred, but the standard array format offers greater flexibility.
...
* The standard JSON array format enables the insertion of multiple edges with discriminators in a single operation.
...
* The *JSON array format* is now the recommended way to handle multi-edges in *4.2*.
====
