...
IMPORTANT: Since v3.9.3, it is no longer required to create a backup for a backups metadata.
A backup’s metadata is now stored as a part of a backup's files and will be searched for with the backups position.

NOTE: If you are using a version previous to v3.9.3, please refer to the instructions below.
