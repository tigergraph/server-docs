...
* If the DR cluster and the primary cluster are deployed in different VPCs, make sure the following ports are open between the clusters:
** `30002` — Kafka Broker port used for data replication between the primary and DR clusters.
** `30003` — MirrorMaker 2 port used by the `infr_mm` connector on the DR cluster to coordinate replication tasks.
