...
[NOTE]
====
By default, `tasks.max` is set to `1` to avoid resource contention when other workloads or concurrent loading jobs run in the cluster.

If your system has available resources, you can increase `tasks.max` to process multiple files in parallel and improve loading throughput.

The effective number of tasks is limited by the number of input files:
