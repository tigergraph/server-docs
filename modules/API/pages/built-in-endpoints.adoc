...
|`new_vertex_only` |No |If `new_vertex_only` is set to true, the request will only insert new vertices and not update existing ones.
Default value is false.

|`vertex_must_exist` |No |If `vertex_must_exist` is true, the request will only insert an *edge* if both the `FROM` and
...
The GET request returns the state of the installation request for each query.

The POST request response included the location of the query under `location`. The `GET` request must include this request ID.
...
| state | Indicates state of installation request for the individual queries. Returns `NEW`,`RUNNING`,`SUCCESS`, or `FAILED`.
| error | Indicates whether an error occurred and what kind of message is included
...
[NOTE]
====
The installation request state shown with `state` will return `SUCCESS` even if some queries fail to install, as long as at least one has successfully installed at the time of response.
====
