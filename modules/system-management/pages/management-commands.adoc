...
See the xref:backup-and-restore:index.adoc[] documentation for a complete guide on using these commands to back up a cluster.
...
Run `gadmin backup create <tag>` to create a backup of the current cluster with a custom tag as the name.
A timestamp is added after the tag.

==== gadmin backup list

Show the list of all previous backups with `gadmin backup list`.

==== gadmin backup remove

Remove a backup from the list of backups with `gadmin backup remove <tag>`. Provide the full tag, including the timestamp.

==== gadmin backup restore

Run `gadmin backup restore <tag>` to restore a backup. Provide the full tag, including the timestamp.
