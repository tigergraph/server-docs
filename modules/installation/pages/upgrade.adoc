...
It is supported only for maintenance updates (for example, 4.2.0 to 4.2.1, or 4.3.1 to 4.3.3). Upgrading from 4.2.0 to 4.3.0 or another major version is not supported.

During a rolling upgrade:

* Loading jobs and running queries will continue to function.
* Occasional slowdowns in queries per second (QPS) may occur.
* TigerGraph does not upgrade Zookeeper (ZK), Kafka, or ETCD as part of this upgrade.
...
* Queries may fail if their execution time exceeds the GPE stop timeout.
By default, `GPE.StopTimeoutMS` is set to 5 minutes; after this time, GPE is forcefully stopped and any incomplete queries will fail.
