...
Upgrading TigerGraph typically consists of three phases:

. Prepare for the upgrade. Review xref:#_before_you_begin[Before you begin].
. Follow the appropriate upgrade procedure for your environment, such as xref:#_upgrading_a_crr_system[Upgrading a CRR system], xref:#_rolling_upgrade[Rolling Upgrade], or the version-specific upgrade paths below.
. Complete xref:#_post_upgrade_checks[Post-Upgrade Checks] after the upgrade finishes.
...
If leaders are upgraded together, total disruption is roughly one leader switch; if upgrading one partition at a time, it scales with the number of partitions.This upgrade strategy is not user-configurable - it is automatically determined and orchestrated by the Controller (CTRL) to optimize efficiency and minimize query disruption based on the cluster topology.
...
[NOTE]
====
When performing a rolling upgrade from a version between 4.2.0 and 4.2.2 to a later 4.2.x version, the upgrade may fail if `Nginx.SSL.Enable=true`.

As a workaround, temporarily disable SSL before starting the rolling upgrade:

[source.wrap,bash]
----
gadmin config set Nginx.SSL.Enable false
gadmin config apply -y
gadmin restart gsql nginx -y
----

After the upgrade completes successfully, re-enable SSL and restart the services:
...
After the upgrade completes, verify that your queries are installed and enabled.

If built-in UDFs or custom UDFs changed during the upgrade, some queries may be disabled and require reinstallation.

For more information, see xref:{page-component-version}@gsql-ref:querying:func/query-user-defined-functions.adoc#_upload_udfs[Query reactivation required if UDFs are modified].
