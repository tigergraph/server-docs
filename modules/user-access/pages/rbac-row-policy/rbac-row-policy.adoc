...
To see a complete list, as well as the xref:tigergraph-server:reference:list-of-privileges-legacy.adoc[legacy privilege keywords] that the object-based privileges correspond to, see the xref:rbac-row-policy/row-policy-privileges-table.adoc[].
